...
export function getProviderWizardDefaults(
  ...
  const envSecrets = processEnv as SecretValueSource
  const safeOpenAIModel =
    sanitizeProviderConfigValue(processEnv.OPENAI_MODEL, envSecrets) ||
    'gpt-4o'
  const safeOpenAIBaseUrl =
    sanitizeProviderConfigValue(processEnv.OPENAI_BASE_URL, envSecrets) ||
    DEFAULT_OPENAI_BASE_URL
  const safeGeminiModel =
    sanitizeProviderConfigValue(processEnv.GEMINI_MODEL, envSecrets) ||
    DEFAULT_GEMINI_MODEL
...
export function buildProfileSaveMessage(
  ...
  lines.push('Restart Deimos to use it.')
...
function buildUsageText(): string {
  ...
  return [
    ...
    '',
    'Choose Auto, Ollama, OpenAI-compatible, Gemini, or Codex, then save a profile for the next Deimos restart.',
  ].join('\n')
}
...
function ProviderChooser({
  ...
  if (summary.savedProfileLabel !== 'none') {
    options.push({
      ...
      value: 'clear',
      description: 'Remove .deimos-profile.json and return to normal startup',
    })
  }
  ...
  return (
    ...
    >
      <Box flexDirection="column" gap={1}>
        <Text>
          Save a provider profile for the next Deimos restart without
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart Deimos to go back to normal startup.`, {
                display: 'system',
              })
            } else {
