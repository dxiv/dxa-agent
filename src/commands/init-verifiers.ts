...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - Deimos Chrome Extension MCP (browser-use via Deimos's Chrome extension)
...
2. **If NO browser automation tools are detected**, ask if they want to install/configure one:
   ...
   - Options to offer:
     ...
     - **Deimos Chrome Extension** - Uses the Deimos Chrome extension for browser interaction (requires the extension installed in Chrome)
...
4. **If user chooses Chrome DevTools MCP or Deimos Chrome Extension**:
   ...
   - For Deimos Chrome Extension, inform them they need the extension installed from the Chrome Web Store
...
**All verifier skills are created in the project root's \`.claude/skills/\` directory.** This ensures they are automatically loaded when Deimos runs in the project.
