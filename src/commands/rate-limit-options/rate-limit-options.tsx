...
import { hasDeimosCloudBillingAccess } from '../../utils/billing.js';
...
function RateLimitOptionsMenu(t0) {
  ...
  bb0: {
    let actionOptions;
    if ($[2] !== deimosUsageLimits.overageDisabledReason || $[3] !== deimosUsageLimits.overageStatus) {
      actionOptions = [];
      if (extraUsage.isEnabled()) {
        const hasBillingAccess = hasDeimosCloudBillingAccess();
        const needsToRequestFromAdmin = isTeamOrEnterprise && !hasBillingAccess;
        const isOrgSpendCapDepleted = deimosUsageLimits.overageDisabledReason === "out_of_credits" || deimosUsageLimits.overageDisabledReason === "org_level_disabled_until" || deimosUsageLimits.overageDisabledReason === "org_service_zero_credit_limit";
        if (needsToRequestFromAdmin && isOrgSpendCapDepleted) {} else {
          const isOverageState = deimosUsageLimits.overageStatus === "rejected" || deimosUsageLimits.overageStatus === "allowed_warning";
          let label;
      ...
      $[2] = deimosUsageLimits.overageDisabledReason;
      $[3] = deimosUsageLimits.overageStatus;
      $[4] = actionOptions;
    } else {
      actionOptions = $[4];
    }
