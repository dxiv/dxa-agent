...
import { DEIMOS_IN_CHROME_MCP_SERVER_NAME, openInChrome } from '../../utils/deimosInChrome/common.js';
...
type Props = {
  ...
  isDeimosCloudSubscriber: boolean;
  isWSL: boolean;
};
function DeimosInChromeMenu(t0) {
  const $ = _c(41);
  const {
    onDone,
        ...
        case "toggle-default":
          {
            ...
            saveGlobalConfig(current => ({
              ...current,
              deimosInChromeDefaultEnabled: newValue
            }));
  ...
  const isDisabled = isWSL || true && !isDeimosCloudSubscriber;
  let t5;
  if ($[18] !== onDone) {
    t5 = () => onDone();
    $[18] = onDone;
  ...
  if ($[20] === Symbol.for("react.memo_cache_sentinel")) {
    t6 = <Text>Deimos in Chrome works with the Chrome extension to let you control your browser directly from Deimos. Navigate websites, fill forms, capture screenshots, record GIFs, and debug with console logs and network requests.</Text>;
    $[20] = t6;
  } else {
    t6 = $[20];
  }
  let t7;
  if ($[21] !== isWSL) {
    t7 = isWSL && <Text color="error">Deimos in Chrome is not supported in WSL at this time.</Text>;
    $[21] = isWSL;
    $[22] = t7;
  } else {
    t7 = $[22];
  ...
  let t12;
  if ($[38] !== t11 || $[39] !== t5) {
    t12 = <Dialog title="Deimos in Chrome (Beta)" onCancel={t5} color="chromeYellow">{t11}</Dialog>;
    $[38] = t11;
    $[39] = t5;
    $[40] = t12;
  } else {
...
function _temp2(c) {
  return c.name === DEIMOS_IN_CHROME_MCP_SERVER_NAME;
}
function _temp(s) {
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  const isSubscriber = isDeimosCloudSubscriber();
  const isWSL = env.isWslEnvironment();
  return <DeimosInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.deimosInChromeDefaultEnabled} isDeimosCloudSubscriber={isSubscriber} isWSL={isWSL} />;
};
