...
describe('onboarding auth precedence cleanup', () => {
  test('clears preexisting OpenAI auth when switching to GitHub', () => {
    const env: NodeJS.ProcessEnv = {
      ...
      DEIMOS_PROVIDER_PROFILE_ENV_APPLIED_ID: 'profile_old',
    }

    applyGithubOnboardingProcessEnv('github:copilot', env)

    expect(env.DEIMOS_USE_GITHUB).toBe('1')
    ...
    expect(env.DEIMOS_USE_OPENAI).toBeUndefined()
    expect(env.DEIMOS_PROVIDER_PROFILE_ENV_APPLIED).toBeUndefined()
    expect(env.DEIMOS_PROVIDER_PROFILE_ENV_APPLIED_ID).toBeUndefined()

    const settingsEnv = buildGithubOnboardingSettingsEnv('github:copilot')
    expect(settingsEnv.DEIMOS_USE_GITHUB).toBe('1')
