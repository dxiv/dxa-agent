...
async function handleBroadcast(
  ...
  if (!teamName) {
    throw new Error(
      'Not in a team context. Create a team with Teammate spawnTeam first, or set DEIMOS_TEAM_NAME.',
    )
  }
  ...
  if (!senderName) {
    throw new Error(
      'Cannot broadcast: sender name is required. Set DEIMOS_AGENT_NAME.',
    )
  }
    ...
    async checkPermissions(input, _context) {
      if (feature('UDS_INBOX') && parseAddress(input.to).scheme === 'bridge') {
        return {
          ...
          message: `Send a message to Remote Control session ${input.to}? It arrives as a user prompt on the receiving Deimos (possibly another machine) via Anthropic's servers.`,
    ...
    async call(input, context, canUseTool, assistantMessage) {
      if (feature('UDS_INBOX') && typeof input.message === 'string') {
        ...
        if (addr.scheme === 'bridge') {
          ...
          const peerSessions = require('../../bridge/peerSessions.js') as {
            postInterClaudeMessage?: (
              target: string,
              message: string,
            ) => Promise<{ ok: boolean; error?: string }>
          }
          const postInterClaudeMessage = peerSessions.postInterClaudeMessage
          /* eslint-enable @typescript-eslint/no-require-imports */
          if (!postInterClaudeMessage) {
            return {
              data: {
                success: false,
                message: `Send to bridge is not available in this build (${input.to})`,
              },
            }
          }
        ...
        if (addr.scheme === 'uds') {
          /* eslint-disable @typescript-eslint/no-require-imports */
          const uds = require('../../utils/udsClient.js') as {
            sendToUdsSocket?: (path: string, message: string) => Promise<void>
          }
          const sendToUdsSocket = uds.sendToUdsSocket
          /* eslint-enable @typescript-eslint/no-require-imports */
          if (!sendToUdsSocket) {
            return {
              data: {
                success: false,
                message: `UDS send is not available in this build (${input.to})`,
              },
            }
          }
          try {
