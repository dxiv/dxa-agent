...
export async function resolveAttachments(
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    // REPL does, at main.tsx init). DEIMOS_BRIEF_UPLOAD lets a host that
    // runs the CLI as a subprocess opt in — e.g. the cowork desktop bridge,
    // which already passes DEIMOS_OAUTH_TOKEN for auth.
    const shouldUpload =
      uploadCtx.replBridgeEnabled ||
      isEnvTruthy(process.env.DEIMOS_BRIEF_UPLOAD)
