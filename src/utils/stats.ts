...
/**
 * Convert a PersistedStatsCache to DeimosCodeStats by computing derived fields.
 */
...
/**
 * Aggregates stats from all Deimos sessions across all projects.
 * Uses a disk cache to avoid reprocessing historical data.
 */
export async function aggregateDeimosCodeStats(): Promise<DeimosCodeStats> {
 ...
 */
export async function aggregateDeimosCodeStatsForRange(
  range: StatsDateRange,
): Promise<DeimosCodeStats> {
  if (range === 'all') {
    return aggregateDeimosCodeStats()
  }
  ...
  return processedStatsToDeimosCodeStats(stats)
}

/**
 * Convert ProcessedStats to DeimosCodeStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToDeimosCodeStats(
...
function getEmptyStats(): DeimosCodeStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
