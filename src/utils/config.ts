...
import { getGlobalDeimosFile } from './env.js'
...
export type ProjectConfig = {
  ...
  hasDeimosMdExternalIncludesApproved?: boolean
  hasDeimosMdExternalIncludesWarningShown?: boolean
...
const DEFAULT_PROJECT_CONFIG: ProjectConfig = {
  ...
  hasDeimosMdExternalIncludesApproved: false,
  hasDeimosMdExternalIncludesWarningShown: false,
}
...
export type GlobalConfig = {
  ...
  deimosCloudMcpEverConnected?: string[]
  ...
  // Deimos usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first Deimos OAuth token
  ...
  // Terminal emulator to launch for deimos:// deep links. Captured from
  ...
  // Deimos in Chrome (browser extension) settings
  hasCompletedDeimosInChromeOnboarding?: boolean // First-run onboarding for Chrome integration
  deimosInChromeDefaultEnabled?: boolean // Whether Chrome integration is enabled by default (undefined = platform default)
  ...
  // Deimos hint protocol state (`<deimos-hint />` in tool output).
  deimosHints?: {
...
export function saveGlobalConfig(
  ...
  try {
    ...
    saveConfig(getGlobalDeimosFile(), written, DEFAULT_GLOBAL_CONFIG)
    writeThroughGlobalConfigCache(written)
  }
}
 ...
 * @internal
 */
function migrateLegacyDeimosConfigKeys(config: GlobalConfig): GlobalConfig {
  const raw = config as GlobalConfig & Record<string, unknown>
  let next: GlobalConfig = { ...config }

  if (
    raw.deimosInChromeDefaultEnabled !== undefined &&
    next.deimosInChromeDefaultEnabled === undefined
  ) {
    next = {
      ...next,
  ...
  if (next.projects) {
    next = {
      ...
      projects: Object.fromEntries(
        Object.entries(next.projects).map(([path, pc]) => {
          const p = pc as ProjectConfig & Record<string, unknown>
          let migrated: ProjectConfig = { ...pc }
          if (
            p.hasDeimosMdExternalIncludesApproved !== undefined &&
            migrated.hasDeimosMdExternalIncludesApproved === undefined
          ) {
            migrated = {
              ...migrated,
              hasDeimosMdExternalIncludesApproved:
                p.hasDeimosMdExternalIncludesApproved as boolean,
            }
          }
          if (
            p.hasDeimosMdExternalIncludesWarningShown !== undefined &&
            migrated.hasDeimosMdExternalIncludesWarningShown === undefined
          ) {
            migrated = {
              ...migrated,
              hasDeimosMdExternalIncludesWarningShown:
                p.hasDeimosMdExternalIncludesWarningShown as boolean,
            }
          }
...
function migrateConfigFields(config: GlobalConfig): GlobalConfig {
  ...
  if (next.installMethod === undefined) {
    ...
    let autoUpdates = next.autoUpdates ?? true
  ...
  return migrateLegacyDeimosConfigKeys(next)
}
...
function startGlobalConfigFreshnessWatcher(): void {
  ...
  const file = getGlobalDeimosFile()
  watchFile(
    file,
...
export function getGlobalConfig(): GlobalConfig {
  ...
  try {
    ...
    try {
      stats = getFsImplementation().statSync(getGlobalDeimosFile())
    } catch {
      // File doesn't exist
    }
    const config = migrateConfigFields(
      getConfig(getGlobalDeimosFile(), createDefaultGlobalConfig),
    )
    globalConfigCache = {
      config,
...
function saveConfigWithLock<A extends object>(
  ...
  try {
    ...
    if (lastReadFileStats && file === getGlobalDeimosFile()) {
      try {
    ...
    if (file === getGlobalDeimosFile() && wouldLoseAuthState(currentConfig)) {
      logForDebugging(
...
function getConfigBackupDir(): string {
  return join(getDeimosConfigHomeDir(), 'backups')
}
...
export function saveCurrentProjectConfig(
  ...
  try {
    ...
    const config = getConfig(getGlobalDeimosFile(), createDefaultGlobalConfig)
...
export function getMemoryPath(memoryType: MemoryType): string {
  ...
  switch (memoryType) {
    case 'User':
      return join(getDeimosConfigHomeDir(), DEIMOS_MEMORY_FILENAME)
    case 'Local':
      return join(cwd, DEIMOS_LOCAL_MEMORY_FILENAME)
    case 'Project':
      return join(cwd, DEIMOS_MEMORY_FILENAME)
    case 'Managed':
      return join(getManagedFilePath(), DEIMOS_MEMORY_FILENAME)
    case 'AutoMem':
...
export function getManagedDeimosRulesDir(): string {
  return join(getManagedFilePath(), DEIMOS_PROJECT_DIR, 'rules')
}

export function getUserDeimosRulesDir(): string {
  return join(getDeimosConfigHomeDir(), 'rules')
}
