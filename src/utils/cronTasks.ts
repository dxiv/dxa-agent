...
const CRON_FILE_REL = join('.deimos', 'scheduled_tasks.json')
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.deimos'), { recursive: true })
 ...
 * and recurring tasks — a recurring task whose window passed while Deimos
