...
function cleanupTerminalModes(skipUnmount: boolean = false): void {
  ...
  try {
    ...
    // Respect DEIMOS_DISABLE_TERMINAL_TITLE — if the user opted out of
    // title changes, don't clear their existing title on exit either.
    if (!isEnvTruthy(process.env.DEIMOS_DISABLE_TERMINAL_TITLE)) {
...
function printResumeHint(): void {
  ...
  if (
    ...
    try {
      ...
      // Don't show resume hint if no session file exists (e.g., subcommands like `deimos /upgrade`)
...
export async function gracefulShutdown(
  ...
  // overall execution via a single budget (DEIMOS_SESSIONEND_HOOKS_TIMEOUT_MS,
