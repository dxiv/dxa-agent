  ...
  try {
    ...
    try {
      ...
      const systemPrompt = asSystemPrompt([
        `You are verifying a stop condition in Deimos. Your task is to verify that the agent completed the given plan. The conversation transcript is available at: ${transcriptPath}\nYou can read this file to analyze the conversation history if needed.
...
- ok: false with reason if the condition is not met`,
      ...
      for await (const message of query({
        ...
        if (message.type === 'attachment') {
          ...
          const parsed = hookResponseSchema().safeParse(att.data)
          if (parsed.success) {
