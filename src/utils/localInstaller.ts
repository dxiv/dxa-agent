...
// hfi.tsx get a chance to set DEIMOS_CONFIG_DIR in main(), and would also
// populate the memoize cache with that stale value for all 150+ other callers.
function getLocalInstallDir(): string {
  return join(getDeimosConfigHomeDir(), 'local')
}
...
/**
 * Install or update Deimos CLI package in the local directory
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install Deimos CLI package: ${result.stderr}`,
      )
      logError(error)
