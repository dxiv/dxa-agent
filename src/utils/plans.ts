...
export const getPlansDirectory = memoize(function getPlansDirectory(): string {
  ...
  if (settingsDir) {
    ...
    if (!resolved.startsWith(cwd + sep) && resolved !== cwd) {
      ...
      plansPath = join(getDeimosConfigHomeDir(), 'plans')
    } else {
      plansPath = resolved
    }
  } else {
...
function recoverPlanFromMessages(log: LogOption): string | null {
  for (let i = log.messages.length - 1; i >= 0; i--) {
    ...
    if (msg.type === 'attachment') {
      ...
      const raw = attachmentMsg.attachment
      if (
        typeof raw === 'object' &&
        raw !== null &&
        'type' in raw &&
        (raw as { type: string }).type === 'plan_file_reference'
      ) {
        const plan = (raw as { planContent?: string }).planContent
        if (typeof plan === 'string' && plan.length > 0) {
          return plan
        }
      }
