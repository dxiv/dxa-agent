...
import { getDeimosCloudOAuthTokens } from '../auth.js'
...
function isPermissionMode(raw: unknown): raw is PermissionMode {
  return typeof raw === 'string' && PERMISSION_MODES.some(m => m === raw)
}
...
export function createChromeContext(
  ...
  return {
    ...
    onAuthenticationError: () => {
      logger.warn(
        'Authentication error occurred. Please ensure you are logged into the Claude browser extension with the same claude.ai account as Deimos.',
      )
    },
    ...
    ...(chromeBridgeUrl && {
      bridgeConfig: {
        ...
        getOAuthToken: async () => {
          return getDeimosCloudOAuthTokens()?.accessToken ?? ''
        },
        ...(isLocalBridge() && { devUserId: 'dev_user_local' }),
      },
    }),
    ...(initialPermissionMode != null ? { initialPermissionMode } : {}),
...
export async function runDeimosInChromeMcpServer(): Promise<void> {
  enableConfigs()
  initializeAnalyticsSink()
  const context = createChromeContext()

  const server = createClaudeForChromeMcpServer(context) as {
