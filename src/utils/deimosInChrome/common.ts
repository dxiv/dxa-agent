...
export const DEIMOS_IN_CHROME_MCP_SERVER_NAME = 'claude-in-chrome'
      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[Deimos in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export function isDeimosInChromeMCPServer(name: string): boolean {
  return normalizeNameForMCP(name) === DEIMOS_IN_CHROME_MCP_SERVER_NAME
}

const MAX_TRACKED_TABS = 200
const trackedTabIds = new Set<number>()

export function trackDeimosInChromeTabId(tabId: number): void {
...
export function isTrackedDeimosInChromeTabId(tabId: number): boolean {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[Deimos in Chrome] No compatible browser found')
    return false
  }
...
function getSocketName(): string {
  // NOTE: This must match the one used in the Deimos in Chrome MCP
