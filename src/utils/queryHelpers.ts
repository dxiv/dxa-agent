  ...
  switch (message.type) {
    ...
    case 'progress': {
      ...
      if (
        ...
        'message' in progressData
      ) {
        const nestedMessage = (progressData as { message: Message }).message
        for (const nm of normalizeMessages([nestedMessage])) {
          if (nm.type === 'assistant') {
            // Skip empty messages (e.g., "(no content)") that shouldn't be output to SDK
            if (!isNotEmptyMessage(nm)) {
              continue
            }
            yield {
              type: 'assistant',
            ...
            yield {
              ...
              isSynthetic: nm.isMeta || nm.isVisibleInTranscriptOnly,
              tool_use_result: nm.mcpMeta
                ? { content: nm.toolUseResult, ...nm.mcpMeta }
                : nm.toolUseResult,
            }
          }
        }
        ...
        const shellProgress = progressData as {
        ...
        if (timeSinceLastSent >= TOOL_PROGRESS_THROTTLE_MS) {
          ...
          yield {
            ...
            tool_name:
              shellProgress.type === 'bash_progress' ? 'Bash' : 'PowerShell',
            parent_tool_use_id: message.parentToolUseID,
            elapsed_time_seconds: shellProgress.elapsedTimeSeconds ?? 0,
            task_id: shellProgress.taskId,
            session_id: getSessionId(),
            uuid: message.uuid,
          }
        }
