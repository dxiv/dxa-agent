 ...
 * =====================
 * This module manages an isolated tmux socket for Deimos's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, Deimos could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started Deimos from within tmux.
 *
 * HOW IT WORKS:
 * 1. Deimos creates its own tmux socket: `deimos-<PID>` (e.g., `deimos-12345`)
 ...
 *    (set in Shell.ts via getDeimosTmuxEnv())
 *
 * This means ANY tmux command run through Deimos - whether via the Tmux tool
 * directly or via Bash - will operate on Deimos's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getDeimosTmuxEnv() returns a value that overrides the
...
const DEIMOS_SOCKET_PREFIX = 'deimos'
...
/**
 * Gets the socket name for Deimos's isolated tmux session.
 * Format: deimos-<PID>
 */
export function getDeimosSocketName(): string {
  if (!socketName) {
    socketName = `${DEIMOS_SOCKET_PREFIX}-${process.pid}`
  }
  return socketName
}
 ...
 */
export function getDeimosSocketPath(): string | null {
  return socketPath
}
...
/**
 * Gets the TMUX environment variable value for Deimos's isolated socket.
 *
 ...
 * initializing the socket. getDeimosTmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
async function doInitialize(): Promise<void> {
  ...
  // Pass DEIMOS_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'DEIMOS_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set DEIMOS_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
  ...
  // Any Deimos instance spawned on this socket will inherit this env var,
