...
const originalSimple = process.env.DEIMOS_SIMPLE
...
async function writeJsonl(entry: unknown): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'deimos-conversation-recovery-hooks-'))
  tempDirs.push(dir)
...
afterEach(async () => {
  mock.restore()
  process.env.DEIMOS_SIMPLE = originalSimple
