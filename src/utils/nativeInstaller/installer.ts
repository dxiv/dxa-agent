...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? 'deimos.exe' : 'deimos'
}
...
function getBaseDirectories() {
  ...
  return {
    ...
    versions: join(getXDGDataHome(), 'deimos', 'versions'),

    // Cache directories (can be deleted)
    staging: join(getXDGCacheHome(), 'deimos', 'staging'),

    // State directories
    locks: join(getXDGStateHome(), 'deimos', 'locks'),
...
async function installVersionFromPackage(
  ...
  try {
    ...
    const nativePackage = entries.find((entry: string) =>
      entry.startsWith('deimos-cli-native-'),
    )
  ...
  if (getPlatform().startsWith('win32')) {
    ...
    try {
      ...
      for (const file of files) {
        if (!/^deimos\.exe\.old\.\d+$/.test(file)) continue
        try {
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, 'deimos.cmd')
      const binPs1 = join(globalPrefix, 'deimos.ps1')
      const binExe = join(globalPrefix, 'deimos')
      ...
      const binSymlink = join(globalPrefix, 'bin', 'deimos')
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Check for local installation at ~/.deimos/local
  const localInstallDir = join(homedir(), '.deimos', 'local')
