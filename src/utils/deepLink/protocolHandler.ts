 ...
 * Protocol Handler
 *
 * Entry point for `--handle-uri <url>`. When the OS invokes the CLI with a
 * `deimos://` URL (or a legacy `claude-cli://` URL passed manually), this module:
 ...
 *
 * @param uri - The raw URI string (e.g., "deimos://open?q=hello+world")
...
export async function handleDeepLinkUri(uri: string): Promise<number> {
  ...
  // baked at registration time, and we want the terminal-launched Deimos to
...
export async function handleUrlSchemeLaunch(): Promise<number | null> {
  ...
  try {
    const { waitForUrlEvent } = (await import(
      'url-handler-napi',
    )) as unknown as typeof import('url-handler-napi') & {
      waitForUrlEvent: (timeoutMs: number) => string | null | undefined
    }
...
/**
 * Resolve the working directory for the launched Deimos instance.
 * Precedence: explicit cwd > repo lookup (MRU clone) > home.
 * A repo that isn't cloned locally is not an error — fall through to home
 * so a web link referencing a repo the user doesn't have still opens Deimos.
 *
