...
afterEach(() => {
  restoreEnv('DEIMOS_USE_GEMINI', originalEnv.DEIMOS_USE_GEMINI)
...
test('still errors when no Gemini credential source is available', async () => {
  ...
  await expect(getProviderValidationError(process.env)).resolves.toBe(
    'GEMINI_API_KEY, GOOGLE_API_KEY, GEMINI_ACCESS_TOKEN, or Google ADC credentials are required when DEIMOS_USE_GEMINI=1.',
  )
})
