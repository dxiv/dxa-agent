...
/** Current provider profile filename (workspace root). */
export const PROFILE_FILE_NAME = '.deimos-profile.json'

/** Legacy filename; still read if present until migrated on save. */
export const LEGACY_PROFILE_FILE_NAME = '.dx' + 'a-agent-profile.json'
...
/** Full `process.env` is accepted wherever we scan `SECRET_ENV_KEYS` for secrets. */
export type SecretValueSourceArg = SecretValueSource | NodeJS.ProcessEnv
...
function resolveProfileFilePath(options?: ProfileFileLocation): string {
  ...
  const primary = resolve(cwd, PROFILE_FILE_NAME)
  const legacy = resolve(cwd, LEGACY_PROFILE_FILE_NAME)
...
function collectSecretValues(
  sources: Array<SecretValueSourceArg | null | undefined>,
): string[] {
...
export async function buildStartupEnvFromProfile(options?: {
  ...
  return buildLaunchEnv({
    ...
    goal:
      options?.goal ??
      normalizeRecommendationGoal(processEnv.DEIMOS_PROFILE_GOAL),
    processEnv,
