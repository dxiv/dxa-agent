...
/**
 * Generates Deimos specific snapshot content
 * This content is always included regardless of user configuration
 */
async function getDeimosCodeSnapshotContent(): Promise<string> {
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in Deimos's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and Deimos content
  const userContent = configFileExists
    ...
    : !isZsh
      ? // we need to manually force alias expansion in bash - normally `getUserSnapshotContent` takes care of this
        'echo "shopt -s expand_aliases" >> "$SNAPSHOT_FILE"'
      : ''
  const deimosCodeSnapshotContent = await getDeimosCodeSnapshotContent()
 ...
 *
 * This function is a critical part of Deimos's shell integration strategy. It:
 *
 ...
 *
 * This approach allows Deimos to execute commands as if they were run in the user's
 ...
 *
 * If the snapshot creation fails (e.g., timeout, permissions issues), the app will still
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Deimos defaults only`,
        )
      }
      ...
      const snapshotsDir = join(getDeimosConfigHomeDir(), 'shell-snapshots')
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.DEIMOS_DONT_INHERIT_ENV
              ? {}
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Deimos home: ${getDeimosConfigHomeDir()}`)
