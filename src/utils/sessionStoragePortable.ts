...
function parseBoundaryLine(
  ...
  try {
    const parsed = JSON.parse(line) as {
      type?: string
      subtype?: string
      compact_metadata?: { preservedSegment?: unknown }
    ...
    const seg =
      parsed.compact_metadata?.preservedSegment ??
      parsed.compactMetadata?.preservedSegment
    return {
      hasPreservedSegment: Boolean(seg),
    }
  } catch {
    return null
  }
