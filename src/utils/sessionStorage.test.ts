...
async function writeJsonl(entries: unknown[]): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'deimos-session-storage-'))
  tempDirs.push(dir)
...
test('stripPersistedToolUseResultsFromJSONLBuffer drops raw toolUseResult while preserving persisted preview content', () => {
  ...
  ;(persisted as { message: UserMessage['message'] }).message = {
    role: 'user',
    content: [
      {
        type: 'tool_result',
  ...
  const blocks = parsed?.message.content as unknown as Array<{ content: string }>
  expect(blocks[0]?.content).toContain('Preview text')
})
...
test('loadTranscriptFile omits raw toolUseResult for persisted-output transcript entries', async () => {
  ...
  const blocks = loaded?.message.content as unknown as Array<{ content: string }>
  expect(blocks[0]?.content).toContain('Preview text')
})
