 ...
 * - Checks environment kind (DEIMOS_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires DEIMOS_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.DEIMOS_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but DEIMOS_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
