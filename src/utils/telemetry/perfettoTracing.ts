/**
 * Perfetto Tracing for Deimos (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via DEIMOS_PERFETTO_TRACE=1 or DEIMOS_PERFETTO_TRACE=<path>
 * 2. Optionally set DEIMOS_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Deimos normally
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.DEIMOS_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    if (isEnvTruthy(envValue)) {
      const tracesDir = join(getDeimosConfigHomeDir(), 'traces')
    ...
    // Start periodic full-trace write if DEIMOS_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.DEIMOS_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
