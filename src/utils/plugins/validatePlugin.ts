...
export async function validatePluginManifest(
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — Deimos ` +
  ...
  if (result.success) {
    ...
    // but the github.com/dxiv/dxa-deimos marketplace sync rejects non-kebab names. Surfacing
    // this here lets authors catch it in CI before the sync fails on them.
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. Deimos accepts ` +
          `it, but the github.com/dxiv/dxa-deimos marketplace sync requires kebab-case ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.deimos-plugin'
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and Deimos ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    ...
    const pluginPath = path.join(absolutePath, '.deimos-plugin', 'plugin.json')
