 ...
 * - CLI flag: --cowork
 * - Environment variable: DEIMOS_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via DEIMOS_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable DEIMOS_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.DEIMOS_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. DEIMOS_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.claude/plugins or ~/.claude/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when DEIMOS_PLUGIN_CACHE_DIR is set via settings.json
  ...
  const envOverride = process.env.DEIMOS_PLUGIN_CACHE_DIR
  if (envOverride) {
    return expandTilde(envOverride)
  }
  return join(getDeimosConfigHomeDir(), getPluginsDirectoryName())
}
 ...
 * image and point DEIMOS_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.DEIMOS_PLUGIN_SEED_DIR
  if (!raw) return []
