 ...
 *
 * getOriginalCwd() (not getCwd()) because "current project" is where Deimos
...
function getPluginVersionFromManifest(
  ...
  const fs = getFsImplementation()
  const manifestPath = join(pluginCachePath, '.deimos-plugin', 'plugin.json')
    ...
    if (existingInstallations && existingInstallations.length > 0) {
      ...
      try {
        ...
        if (typeof entry.source === 'string') {
          ...
          if (dirEntries.includes('.deimos-plugin')) {
