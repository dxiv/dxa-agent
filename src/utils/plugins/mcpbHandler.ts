...
import {
  ...
  type McpbManifest as ParsedMcpbManifest,
} from '../dxt/helpers.js'
...
/** Field schema for manifest `user_config` entries. */
...
/**
 * Full MCPB manifest for this module. `dxt/helpers` keeps a minimal parsed
 * type; runtime JSON always includes the fields we use below.
 */
export type McpbManifest = ParsedMcpbManifest & {
  version: string
  server: unknown
  user_config?: UserConfigSchema
}

async function parseFullMcpbManifest(data: Uint8Array): Promise<McpbManifest> {
  return (await parseAndValidateManifestFromBytes(data)) as McpbManifest
}
...
async function generateMcpConfig(
  ...
  const mcpb = (await import('@anthropic-ai/mcpb')) as {
    getMcpConfigForManifest?: (opts: {
      manifest: McpbManifest
      extensionPath: string
      systemDirs: ReturnType<typeof getSystemDirectories>
      userConfig: UserConfigValues
      pathSeparator: string
    }) => Promise<McpServerConfig | null | undefined>
  }
  const getMcpConfigForManifest = mcpb.getMcpConfigForManifest
  if (!getMcpConfigForManifest) {
    throw new Error(
      '@anthropic-ai/mcpb is missing getMcpConfigForManifest — update or install the package',
    )
  }
...
export async function loadMcpbFile(
  ...
  if (metadata && !(await checkMcpbChanged(source, pluginPath))) {
    ...
    const manifest = await parseFullMcpbManifest(manifestData)
