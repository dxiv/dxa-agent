/**
 * Marketplace manager for Deimos plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a Deimos bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Pulls latest changes with a configurable timeout (default 120s, override via DEIMOS_PLUGIN_GIT_TIMEOUT_MS).
...
function getPluginGitTimeoutMs(): number {
  const envValue = process.env.DEIMOS_PLUGIN_GIT_TIMEOUT_MS
  if (envValue) {
...
function enhanceGitPullErrorMessages(result: {
  ...
  if (result.error?.includes('timed out')) {
    ...
    return {
      ...result,
      stderr: `Git pull timed out after ${timeoutSec}s. Try increasing the timeout via DEIMOS_PLUGIN_GIT_TIMEOUT_MS environment variable.\n\nOriginal error: ${result.stderr}`,
    }
  }
 ...
 *
 * Clones a git repository with a configurable timeout (default 120s, override via DEIMOS_PLUGIN_GIT_TIMEOUT_MS)
...
export async function gitClone(
  ...
  if (result.error?.includes('timed out')) {
    return {
      ...result,
      stderr: `Git clone timed out after ${Math.round(timeoutMs / 1000)}s. The repository may be too large for the current timeout. Set DEIMOS_PLUGIN_GIT_TIMEOUT_MS to increase it (e.g., 300000 for 5 minutes).\n\nOriginal error: ${result.stderr}`,
    }
  }
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'Deimos-Code-Plugin-Manager',
  }
  ...
  try {
    switch (source.source) {
      ...
      case 'directory': {
        ...
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.deimos-plugin', 'marketplace.json')
...
async function readCachedMarketplace(
  ...
  const nestedPath = join(installLocation, '.deimos-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `Deimos version. Run 'claude marketplace remove ${name}' and ` +
  ...
  try {
    ...
    if (source.source === 'github' || source.source === 'git') {
      ...
      if (source.source === 'github') {
        ...
        if (isEnvTruthy(process.env.DEIMOS_REMOTE)) {
      ...
      try {
        ...
        const reason =
          name === 'deimos-plugins'
            ? `We've deprecated "deimos-plugins" in favor of "claude-plugins-official".`
