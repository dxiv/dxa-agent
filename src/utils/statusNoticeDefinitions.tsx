...
import { getLargeMemoryFiles, MAX_MEMORY_CHARACTER_COUNT, type MemoryFileInfo } from './deimosmd.js';
import figures from 'figures';
import { getCwd } from './cwd.js';
import { relative } from 'path';
import { formatNumber } from './format.js';
import type { getGlobalConfig } from './config.js';
import { getAnthropicApiKeyWithSource, getApiKeyFromConfigOrMacOSKeychain, getAuthTokenSource, isDeimosCloudSubscriber } from './auth.js';
...
const deimosCloudSubscriberExternalTokenNotice: StatusNoticeDefinition = {
  id: 'deimos-cloud-external-token',
  type: 'warning',
  isActive: () => {
    const authTokenInfo = getAuthTokenSource();
    return isDeimosCloudSubscriber() && (authTokenInfo.source === 'ANTHROPIC_AUTH_TOKEN' || authTokenInfo.source === 'apiKeyHelper');
  },
  render: () => {
    const authTokenInfo = getAuthTokenSource();
    return <Box flexDirection="row" marginTop={1}>
        <Text color="warning">{figures.warning}</Text>
        <Text color="warning">
          Auth conflict: Using {authTokenInfo.source} instead of Deimos Cloud
  ...
  render: () => {
    ...
    return <Box flexDirection="row" marginTop={1}>
        ...
        <Text color="warning">
          ...
          Either unset {apiKeySource}, or run `deimos /logout`.
        </Text>
      </Box>;
  }
};
  ...
  render: () => {
    ...
    return <Box flexDirection="column" marginTop={1}>
        ...
        <Box flexDirection="column" marginLeft={3}>
          <Text color="warning">
            · Trying to use{' '}
            {authTokenInfo.source === 'deimos.cloud'
              ? 'Deimos Cloud'
              : authTokenInfo.source}
            ?{' '}
            {apiKeySource === 'ANTHROPIC_API_KEY' ? 'Unset the ANTHROPIC_API_KEY environment variable, or run /logout then say "No" to the API key approval before login.' : apiKeySource === 'apiKeyHelper' ? 'Unset the apiKeyHelper setting.' : 'run /logout'}
          </Text>
          <Text color="warning">
            · Trying to use {apiKeySource}?{' '}
            {authTokenInfo.source === 'deimos.cloud'
              ? 'Run /logout to sign out of Deimos Cloud (web subscription).'
  ...
  render: () => {
    ...
    return <Box flexDirection="row" gap={1} marginLeft={1}>
        ...
        <Text>
          ...
          <Text bold>https://github.com/dxiv/dxa-deimos</Text>
        </Text>
      </Box>;
  }
};
...
export const statusNoticeDefinitions: StatusNoticeDefinition[] = [largeMemoryFilesNotice, largeAgentDescriptionsNotice, deimosCloudSubscriberExternalTokenNotice, apiKeyConflictNotice, bothAuthMethodsNotice, jetbrainsPluginNotice];
