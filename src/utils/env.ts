...
// Config and data paths — global state under ~/.deimos/deimos*.json
export const getGlobalDeimosFile = memoize((): string => {
  const filename = `deimos${fileSuffixForOauthConfig()}.json`
  return join(getDeimosConfigHomeDir(), filename)
})
 ...
 * If DEIMOS_HOST_PLATFORM is set to a valid platform value, that overrides
...
export function getHostPlatformForAnalytics(): Platform {
  const override = process.env.DEIMOS_HOST_PLATFORM
