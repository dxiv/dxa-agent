 ...
 *
 * When DEIMOS_USE_OPENAI=1 (or legacy DEIMOS_USE_OPENAI=1),
 * getContextWindowForModel() falls through to
...
const OPENAI_CONTEXT_WINDOWS: Record<string, number> = {
  ...
  // Groq (fast inference) — see https://console.groq.com/docs/models
  ...
  // Alibaba DashScope (OpenAI-compatible) — qwen-plus family ~128k class
  'qwen-max':                 128_000,
  'qwen-plus':                128_000,
  'qwen-turbo':               128_000,
  ...
  // OpenRouter / multi-vendor slugs (prefix match — longer keys listed first)
  'qwen/qwen3-235b-a22b':     131_072,
  'qwen/qwen2.5-72b-instruct': 128_000,
  'qwen/qwen2.5-32b-instruct': 128_000,
  'qwen/qwen2.5-coder-32b':    128_000,

  // Together AI (preset uses Qwen/* caps; add common chat IDs)
  ...
  // Google (native via DEIMOS_USE_GEMINI)
...
const OPENAI_MAX_OUTPUT_TOKENS: Record<string, number> = {
  ...
  // Perplexity Sonar
  'sonar-deep-research':      32_768,
  'sonar-reasoning-pro':      32_768,
  'sonar-pro':                32_768,
  'sonar':                    16_384,

  // OpenRouter Qwen slugs
  'qwen/qwen3-235b-a22b':     32_768,
  'qwen/qwen2.5-72b-instruct': 16_384,
  'qwen/qwen2.5-32b-instruct': 16_384,
  'qwen/qwen2.5-coder-32b':    16_384,

  // Together AI
  'Qwen/Qwen3.5-9B':          16_384,
  'Qwen/Qwen2.5-72B-Instruct-Turbo': 16_384,
