...
/** Control-plane SDK surface (types omit `BedrockClient` in some installs). */
type BedrockControlPackage = {
  BedrockClient: new (config: Record<string, unknown>) => {
  ...
  ListInferenceProfilesCommand: new (input: Record<string, unknown>) => unknown
  GetInferenceProfileCommand: new (input: Record<string, unknown>) => unknown
}

async function importBedrockControl(): Promise<BedrockControlPackage> {
  return (await import('@aws-sdk/client-bedrock')) as unknown as BedrockControlPackage
}
...
async function createBedrockClient() {
  const { BedrockClient } = await importBedrockControl()
  ...
  const skipAuth = isEnvTruthy(process.env.DEIMOS_SKIP_BEDROCK_AUTH)
