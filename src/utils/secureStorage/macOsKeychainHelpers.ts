...
/**
 * Get the service/resource name for secure storage, scoped by DEIMOS_CONFIG_DIR
...
export function getSecureStorageServiceName(
  ...
  const isDefaultDir = !process.env.DEIMOS_CONFIG_DIR
  ...
  return `Deimos${getOauthConfig().OAUTH_FILE_SUFFIX}${serviceSuffix}${dirHash}`
}
...
//
// The sync read() path takes ~500ms per `security` spawn. With 50+ github.com/dxiv/dxa-deimos
