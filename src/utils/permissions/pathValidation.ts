...
export function isPathInSandboxWriteAllowlist(resolvedPath: string): boolean {
  ...
  const { allowOnly, denyWithinAllow } = SandboxManager.getFsWriteConfig() as {
    allowOnly: string[]
...
export function isPathAllowed(
  ...
  // Checks: Windows patterns, Deimos config files, dangerous files (on original + symlink paths)
  if (operationType !== 'read') {
