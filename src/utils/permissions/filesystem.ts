...
import { getDeimosConfigHomeDir } from '../envUtils.js'
import {
  getFsImplementation,
...
/**
 * If filePath is inside a .deimos/skills/{name}/ directory (project or global),
 ...
 */
export function getDeimosSkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.deimos', 'skills')),
      prefix: '/.deimos/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.deimos', 'skills')),
      prefix: '~/.deimos/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.deimos/skills/*/**' which matches ALL skills. Return null
...
// Always ask when Deimos tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const commandsDir = join(getOriginalCwd(), '.deimos', 'commands')
  const agentsDir = join(getOriginalCwd(), '.deimos', 'agents')
  const skillsDir = join(getOriginalCwd(), '.deimos', 'skills')
 ...
 * The scratchpad is a per-session directory for Deimos to write temporary files.
...
/**
 * Returns the user-specific Deimos temp directory name.
 * On Unix: 'deimos-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'deimos' (tmpdir() is already per-user)
 */
export function getDeimosTempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'deimos'
  }
...
/**
 * Returns the Deimos temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/deimos-{uid}/ (resolved to /private/tmp/deimos-{uid}/ on macOS)
 * - On Windows: {tmpdir}/deimos/ (e.g., C:\Users\{user}\AppData\Local\Temp\deimos\)
 * This is a per-user temporary directory used by Deimos for all temp files.
 *
 ...
 * resolution, paths like /tmp/deimos-{uid}/... wouldn't match /private/tmp/deimos-{uid}/...
 */
// Memoized: called per-tool from permission checks (yoloClassifier, sandbox-adapter)
// and per-turn from BashTool prompt. Inputs (DEIMOS_TMPDIR env + platform) are
// fixed at startup, and the realpath of the system tmp dir does not change mid-session.
export const getDeimosTempDir = memoize(function getDeimosTempDir(): string {
  const baseTmpDir =
    process.env.DEIMOS_TMPDIR ||
  ...
  return join(resolvedBaseTmpDir, getDeimosTempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getDeimosTempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)

/**
 * Returns the project temp directory path with trailing separator.
 * Path format: /tmp/deimos-{uid}/{sanitized-cwd}/
 */
export function getProjectTempDir(): string {
  return join(getDeimosTempDir(), sanitizePath(getOriginalCwd())) + sep
}

/**
 * Returns the scratchpad directory path for the current session.
 * Path format: /tmp/deimos-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/deimos-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .claude/worktrees/ is a structural path (where Deimos stores
 ...
 * - Deimos config files (.claude/settings.json, .claude/commands/, .claude/agents/)
 * - MCP CLI state files (managed internally by Deimos)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `Deimos requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for Deimos config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `Deimos requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `Deimos requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `Deimos requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `Deimos requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `Deimos requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `Deimos requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // "allow Deimos to edit its own settings for this session" option actually works.
  ...
  // 1.7. Check comprehensive safety validations (Windows patterns, Deimos config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    ...
    const skillScope = getDeimosSkillScope(path)
...
export function checkEditableInternalPath(
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      const jobsRoot = join(getDeimosConfigHomeDir(), 'jobs')
  ...
  // The desktop's preview_start MCP tool instructs Deimos to create/update
  ...
  if (
    ...
    normalizeCaseForComparison(join(getOriginalCwd(), '.deimos', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Project temp directory (/tmp/deimos-{uid}/{sanitized-cwd}/)
  ...
  const tasksDir = join(getDeimosConfigHomeDir(), 'tasks') + sep
  if (
  ...
  const teamsReadDir = join(getDeimosConfigHomeDir(), 'teams') + sep
  if (
