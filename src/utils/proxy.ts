...
function createHttpsProxyAgent(
  ...
  if (isEnvTruthy(process.env.DEIMOS_PROXY_RESOLVES_HOSTS)) {
 ...
 *   env var is set by `deimos ssh` on the remote CLI to route API calls through
...
export function getProxyFetchOptions(opts?: { forAnthropicAPI?: boolean }): {
  ...
  // ANTHROPIC_UNIX_SOCKET tunnels through the `deimos ssh` auth proxy, which
