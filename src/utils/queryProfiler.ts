 ...
 * pipeline from user input to first token arrival. Enable by setting DEIMOS_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.DEIMOS_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set DEIMOS_PROFILE_QUERY=1)'
  }
