...
// Deimos configuration directory names (under .deimos/)
export const DEIMOS_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type DeimosConfigDirectory = (typeof DEIMOS_CONFIG_DIRECTORIES)[number]
...
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.deimos', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDir = join(getDeimosConfigHomeDir(), subdir)
    const managedDir = join(getManagedFilePath(), '.deimos', subdir)
  ...
  (subdir: DeimosConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
