  ...
  for await (const hookResult of executeUserPromptSubmitHooks(
    ...
    if (hookResult.message) {
      ...
      if (msg.type === 'attachment') {
        ...
        switch (att.type) {
          case 'hook_success':
            ...
            result.messages.push({
              ...
              attachment: {
                ...att,
                content: applyTruncation(att.content),
              },
            })
            break
          default:
        ...
        result.messages.push(
          msg as ProcessUserInputBaseResult['messages'][number],
        )
      }
    }
  }
...
async function processUserInputBase(
  ...
  // Store images to disk so Deimos can reference the path in context
  ...
  if (inputString !== null && mode === 'prompt') {
    ...
    const agentMention = attachmentMessages.find(
      (m): m is AttachmentMessage & { attachment: AgentMentionAttachment } =>
        ...
        m.attachment !== null &&
        (m.attachment as AgentMentionAttachment).type === 'agent_mention',
    )
