...
export function getSessionEndHookTimeoutMs(): number {
  const raw = process.env.DEIMOS_SESSIONEND_HOOKS_TIMEOUT_MS
 ...
 * (POSIX path conversion, .sh auto-prepend, DEIMOS_SHELL_PREFIX).
...
async function execCommandHook(
  ...
  // DEIMOS_SHELL_PREFIX wraps the command via POSIX quoting
  ...
  // a DEIMOS_PS_SHELL_PREFIX (or shell-aware prefix) is a follow-up.
  const finalCommand =
    !isPowerShell && process.env.DEIMOS_SHELL_PREFIX
      ? formatShellPrefixCommand(process.env.DEIMOS_SHELL_PREFIX, command)
      : command
    ...
    try {
      ...
      if (hook.type === 'prompt') {
        ...
        if (promptResult.message?.type === 'attachment') {
          const att = promptResult.message.attachment as {
            type?: string
            command?: string
            durationMs?: number
          }
      ...
      if (hook.type === 'agent') {
        ...
        if (agentResult.message?.type === 'attachment') {
          const att = agentResult.message.attachment as {
            type?: string
            command?: string
            durationMs?: number
          }
 ...
 * - Lazy load when Deimos touches a file that triggers nested CLAUDE.md or
