 ...
 *
 * Usage:
 *   deimos --provider openai --model gpt-4o
 *   deimos --provider gemini --model gemini-2.0-flash
 *   deimos --provider ollama --model llama3.2
 *   deimos --provider anthropic   (default, no-op)
 */
 ...
 * Sets the required DEIMOS_USE_* flag and any provider-specific
...
export function applyProviderFlag(
  ...
  switch (provider as ProviderFlagName) {
    ...
    case 'openai':
      ...
      // Back-compat: older builds/scripts.
      process.env.DEIMOS_USE_OPENAI ??= '1'
    ...
    case 'bedrock':
      process.env.DEIMOS_USE_BEDROCK = '1'
      break

    case 'vertex':
      process.env.DEIMOS_USE_VERTEX = '1'
      break
