...
import type { SDKAssistantMessage } from 'src/entrypoints/sdk/coreTypes.js'
  ...
  return function transformToStreamlined(
    ...
    switch (message.type) {
      case 'assistant': {
        ...
        const content = (assistant.message as { content?: unknown }).content
        const text = Array.isArray(content)
          ? extractTextContent(content, '\n').trim()
          : ''

        // Accumulate tool counts from this message
        accumulateToolUses(assistant, cumulativeCounts)
