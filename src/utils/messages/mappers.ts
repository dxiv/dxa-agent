...
import type { DeimosUsageLimits } from 'src/services/subscriberUsageLimits.js'
          ...
          return [
            {
              ...
              compact_metadata: fromSDKCompactMetadata(
                compactMsg.compact_metadata,
              ),
              uuid: compactMsg.uuid,
              timestamp: new Date().toISOString(),
            } as Message,
          ]
        }
    ...
    switch (message.type) {
      ...
      case 'system':
          ...
          return [
            {
              ...
              session_id: getSessionId(),
              uuid: message.uuid,
              compact_metadata: toSDKCompactMetadata(message.compact_metadata),
            },
          ]
        }
        ...
        if (message.subtype === 'local_command') {
          const local = message as SystemLocalCommandMessage & { content: string }
          if (
            local.content.includes(`<${LOCAL_COMMAND_STDOUT_TAG}>`) ||
            local.content.includes(`<${LOCAL_COMMAND_STDERR_TAG}>`)
          ) {
            return [
...
/**
 * Maps internal DeimosUsageLimits to the SDK-facing SDKRateLimitInfo type,
 * stripping internal-only fields like unifiedRateLimitFallbackAvailable.
 */
export function toSDKRateLimitInfo(
  limits: DeimosUsageLimits | undefined,
