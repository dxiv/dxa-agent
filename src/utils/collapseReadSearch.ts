    ...
    const info = getSearchOrReadFromContent(
      firstContent
        ? {
            ...
            name: msg.toolName,
            input:
              firstContent.type === 'tool_use' ? firstContent.input : undefined,
          }
        : undefined,
      tools,
    )
  ...
  for (const msg of messages) {
    if (isCollapsibleToolUse(msg, tools)) {
      ...
      if (toolInfo.isMemoryWrite) {
        ...
        if (input?.command) {
          // Prefer the stripped `# comment` if present (it's what Deimos wrote
      ...
      msg.type === 'attachment' &&
      (msg.attachment as { type?: string }).type === 'relevant_memories'
    ) {
      ...
      const relevant = msg.attachment as {
        type: 'relevant_memories'
        memories: { path: string; content: string; mtimeMs: number }[]
      }
      currentGroup.relevantMemories ??= []
      currentGroup.relevantMemories.push(...relevant.memories)
      ...
      if (
        ...
        !(
          msg.type === 'attachment' &&
          (msg.attachment as { type?: string }).type === 'nested_memory'
        )
      ) {
