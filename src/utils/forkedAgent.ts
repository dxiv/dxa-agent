...
import { accumulateUsage, updateUsage } from '../services/api/anthropicMessages.js'
  ...
  try {
    for await (const message of query({
      ...
      if (message.type === 'stream_event') {
        const streamEvt = message.event as {
          type?: string
          usage?: BetaMessageDeltaUsage
        }
        if (
          streamEvt.type === 'message_delta' &&
          streamEvt.usage !== undefined
        ) {
          const turnUsage = updateUsage({ ...EMPTY_USAGE }, streamEvt.usage)
