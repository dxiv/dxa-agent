...
function ripGrepRaw(
  ...
  const parsedSeconds =
    parseInt(process.env.DEIMOS_GLOB_TIMEOUT_SECONDS || '', 10) || 0
...
async function ripGrepFileCount(
  ...
  return new Promise<number>((resolve, reject) => {
    ...
    child.on('error', err => {
      ...
      reject(
        e.code === 'ENOENT' ? wrapRipgrepUnavailableError(e) : err,
      )
    })
  })
}
...
export async function ripGrep(
  ...
  return new Promise((resolve, reject) => {
    const handleResult = (
      ...
      if (CRITICAL_ERROR_CODES.includes(String(error.code))) {
        reject(
          error.code === 'ENOENT'
            ? wrapRipgrepUnavailableError(error as RipgrepErrorLike)
            : error,
        )
        return
      }
      ...
      // If we timed out with no results, throw an error so Deimos knows the search
