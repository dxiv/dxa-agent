...
import { getAccountInformation, isDeimosCloudSubscriber } from './auth.js';
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './deimosmd.js';
import { getDoctorDiagnostic } from './doctorDiagnostic.js';
import { getAWSRegion, getDefaultVertexRegion, isEnvTruthy } from './envUtils.js';
import { getDisplayPath } from './file.js';
import { formatNumber } from './format.js';
import { getIdeClientName, type IDEExtensionInstallationStatus, isJetBrainsIde, toIDEDisplayName } from './ide.js';
import { getDeimosCloudUserDefaultModelDescription, modelDisplayString } from './model/model.js';
...
export function buildSandboxProperties(): Property[] {
  const sandboxBackend: string = 'external'
  if (sandboxBackend !== 'ant') {
    return [];
  }
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.DEIMOS_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.DEIMOS_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.DEIMOS_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.DEIMOS_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.DEIMOS_CLIENT_KEY
      });
    }
  }
  return properties;
...
export function getModelDisplayLabel(mainLoopModel: string | null): string {
  ...
  if (mainLoopModel === null && isDeimosCloudSubscriber()) {
    const description = getDeimosCloudUserDefaultModelDescription();
