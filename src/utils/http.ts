...
import { getDeimosUserAgent, getDeimosVersionString } from './userAgent.js'
...
export function getUserAgent(): string {
  ...
  return `deimos/${getDeimosVersionString()} (${process.env.USER_TYPE}, ${process.env.DEIMOS_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.DEIMOS_ENTRYPOINT) {
    parts.push(process.env.DEIMOS_ENTRYPOINT)
  }
  ...
  return `deimos/${getDeimosVersionString()}${suffix}`
}

// User-Agent for WebFetch: identifies Deimos CLI-initiated fetches (distinct from browser bots).
export function getWebFetchUserAgent(): string {
  const supportUrl =
    getAPIProvider() === 'firstParty'
      ? 'https://support.anthropic.com/'
      : 'https://github.com/dxiv/dxa-deimos'
  return `Deimos-User (${getDeimosUserAgent()}; +${supportUrl})`
}
...
export async function withOAuth401Retry<T>(
  ...
  try {
    ...
    const failedAccessToken = getDeimosCloudOAuthTokens()?.accessToken
