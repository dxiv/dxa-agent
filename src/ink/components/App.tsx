...
export default class App extends PureComponent<Props, State> {
  ...
  stdinMode: 'readable' | 'data' = process.env.DEIMOS_USE_READABLE_STDIN === '1' ? 'readable' : 'data';
  ...
  override componentDidMount() {
    ...
    if (this.props.stdout.isTTY && !isEnvTruthy(process.env.DEIMOS_ACCESSIBILITY)) {
  ...
  handleSuspend = (): void => {
    ...
    // Emit suspend event for Deimos to handle. Mostly just has a notification
    ...
    const resumeHandler = () => {
      ...
      if (this.props.stdout.isTTY) {
        if (!isEnvTruthy(process.env.DEIMOS_ACCESSIBILITY)) {
          this.props.stdout.write(HIDE_CURSOR);
        }
      ...
      // Emit resume event for Deimos to handle
