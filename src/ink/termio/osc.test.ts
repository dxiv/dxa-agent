...
const mockedClipboardPath = join(process.cwd(), 'deimos-clipboard.txt')
...
type ExecFileCall = [string, string[], ...unknown[]]
...
async function waitForExecCall(
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    const call = execFileNoThrowMock.mock.calls.find(
      c => (c as unknown as ExecFileCall)[0] === command,
    ) as unknown as ExecFileCall | undefined
    if (call) {
      return call
    }
...
describe('Windows clipboard fallback', () => {
  ...
  test('uses PowerShell instead of clip.exe for local Windows copy', async () => {
    ...
    expect(
      execFileNoThrowMock.mock.calls.some(
        c => (c as unknown as ExecFileCall)[0] === 'clip',
      ),
    ).toBe(false)
    expect(
      execFileNoThrowMock.mock.calls.some(
        c => (c as unknown as ExecFileCall)[0] === 'powershell',
      ),
    ).toBe(true)
  })
  ...
  test('passes Windows clipboard text through a UTF-8 temp file instead of stdin', async () => {
    ...
    const winOpts = windowsCall?.[2] as Record<string, unknown> | undefined
    expect(winOpts).toMatchObject({
      stdin: 'ignore',
    })
    expect(winOpts).not.toMatchObject({ input: 'Привет мир' })
    expect(winOpts).not.toMatchObject({
      env: expect.objectContaining({
        DEIMOS_CLIPBOARD_TEXT_B64: expect.any(String),
      }),
    })
...
describe('clipboard path behavior remains stable', () => {
  ...
  test('local macOS clipboard fallback still uses pbcopy', async () => {
    ...
    expect(
      execFileNoThrowMock.mock.calls.some(
        c => (c as unknown as ExecFileCall)[0] === 'pbcopy',
      ),
    ).toBe(true)
  })
})
