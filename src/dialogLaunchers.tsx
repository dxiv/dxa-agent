...
export async function launchSnapshotUpdateDialog(root: Root, props: {
  ...
  const SnapshotUpdate = SnapshotUpdateDialog as React.ComponentType<SnapshotUpdateDialogProps>;
  return showSetupDialog<'merge' | 'keep' | 'replace'>(root, done => <SnapshotUpdate agentType={props.agentType} scope={props.scope} snapshotTimestamp={props.snapshotTimestamp} onComplete={done} onCancel={() => done('keep')} />);
}
...
/**
 * `deimos assistant` found zero sessions — show the same install wizard
