 ...
 * 1. SSETransport (SSE reads + POST writes) when DEIMOS_USE_CCR_V2 is set
 * 2. HybridTransport (WS reads + POST writes) when DEIMOS_POST_FOR_SESSION_INGRESS_V2 is set
...
export function getTransportForUrl(
  ...
  if (url.protocol === 'ws:' || url.protocol === 'wss:') {
    if (isEnvTruthy(process.env.DEIMOS_POST_FOR_SESSION_INGRESS_V2)) {
