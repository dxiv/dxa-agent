...
export function accumulateStreamEvents(
  ...
  for (const msg of buffer) {
    ...
    switch (eventType) {
      case 'message_start': {
        const startMessage = (ev as { message?: unknown }).message
        if (
          typeof startMessage !== 'object' ||
          startMessage === null ||
          !('id' in startMessage) ||
          typeof (startMessage as { id: unknown }).id !== 'string'
        ) {
          out.push(msg)
          break
        }
        const id = (startMessage as { id: string }).id
      ...
      case 'content_block_delta': {
        ...
        const indexRaw = (ev as { index?: unknown }).index
 ...
 * - Epoch management: reads worker_epoch from DEIMOS_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (DEIMOS_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * DEIMOS_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    DEIMOS_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.DEIMOS_WORKER_EPOCH
  ...
  async writeEvent(message: StdoutMessage): Promise<void> {
    ...
    if (message.type === 'assistant') {
      ...
      if (
        ...
        clearStreamAccumulatorForMessage(this.streamTextAccumulator, {
          session_id: message.session_id,
          parent_tool_use_id: message.parent_tool_use_id,
          message: { id: (m as { id: string }).id },
        })
      }
    }
