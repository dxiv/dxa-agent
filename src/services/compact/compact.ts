...
// Skills can be large (verify=18.7KB, anthropic-api≈20KB). Previously re-injected
...
export function stripReinjectedAttachments(messages: Message[]): Message[] {
  if (feature('EXPERIMENTAL_SKILL_SEARCH')) {
    return messages.filter(m => {
      ...
      const t = (m.attachment as { type?: string }).type
      return !(t === 'skill_discovery' || t === 'skill_listing')
    })
  }
  return messages
}
...
export async function compactConversation(
  ...
  try {
    ...
    if (preCompactDiscovered.size > 0 && boundaryMarker.compact_metadata) {
      boundaryMarker.compact_metadata.preCompactDiscoveredTools = [
