...
import { getMaxOutputTokensForModel } from '../api/anthropicMessages.js'
...
export function getEffectiveContextWindowSize(model: string): number {
  ...
  const autoCompactWindow = process.env.DEIMOS_AUTO_COMPACT_WINDOW
  ...
  const effective = contextWindow - reservedTokensForSummary
  // Belt-and-suspenders: bad window/max-out pairings must not yield ≤0 effective
  // (breaks blocking-limit math and can force immediate "context limit" errors).
  return Math.max(1024, effective)
}
...
/**
 * When the effective window is small (e.g. unknown OpenAI-compat models use an
 * 8k conservative window and output reservation leaves ~1k effective),
 * `effective - AUTOCOMPACT_BUFFER` is ≤0. Then `tokenUsage >= threshold` is
 * true for every non‑negative usage, so autocompact runs on the first "hello"
 * and the token footer shows nonsense (~"100% until auto-compact").
 */
function clampAutoCompactThreshold(
  ...
  return Math.max(1, Math.floor(effectiveContextWindow * 0.88))
}
...
export function getAutoCompactThreshold(model: string): number {
  ...
  if (envPercent) {
    ...
    if (!isNaN(parsed) && parsed > 0 && parsed <= 100) {
      ...
      return clampAutoCompactThreshold(
        effectiveContextWindow,
        Math.min(percentageThreshold, autocompactThreshold),
      )
    }
  }
...
export function calculateTokenWarningState(
  ...
  // For tiny thresholds, fixed 20k buffers go negative and light the warning bar
  // on every turn. Scale buffers down with the active threshold (same base as
  // percentLeft above: autocompact threshold when enabled, else full window).
  const warningBuffer = Math.min(
    WARNING_THRESHOLD_BUFFER_TOKENS,
    Math.max(256, Math.floor(threshold * 0.25)),
  )
  const errorBuffer = Math.min(
    ERROR_THRESHOLD_BUFFER_TOKENS,
    Math.max(128, Math.floor(threshold * 0.15)),
  )
  ...
  const blockingLimitOverride = process.env.DEIMOS_BLOCKING_LIMIT_OVERRIDE
