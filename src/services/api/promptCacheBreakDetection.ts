...
import { getDeimosTempDir } from 'src/utils/permissions/filesystem.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getDeimosTempDir(), `cache-break-${suffix}.diff`)
}
...
type PreviousState = {
  ...
  effortValue: string
  /** Hash of getExtraBodyParams() — catches DEIMOS_EXTRA_BODY and
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getDeimosTempDir(), { recursive: true })
