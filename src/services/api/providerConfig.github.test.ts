...
const originalUseGithub = process.env.DEIMOS_USE_GITHUB

afterEach(() => {
  if (originalUseGithub === undefined) {
    delete process.env.DEIMOS_USE_GITHUB
  } else {
    process.env.DEIMOS_USE_GITHUB = originalUseGithub
  }
})
...
test('resolveProviderRequest applies GitHub normalization when DEIMOS_USE_GITHUB=1', () => {
