...
import { markDeimosCloudMcpConnected } from './anthropicCloudMcp.js'
...
const deimosInChromeToolRendering =
  (): typeof import('../../utils/deimosInChrome/toolRendering.js') =>
    require('../../utils/deimosInChrome/toolRendering.js')
...
function getMcpAuthCachePath(): string {
  return join(getDeimosConfigHomeDir(), 'mcp-needs-auth-cache.json')
}
...
export function createDeimosCloudProxyFetch(innerFetch: FetchLike): FetchLike {
  return async (url, init) => {
    const doRequest = async () => {
      ...
      // Return the exact token that was sent. Reading getDeimosCloudOAuthTokens()
    ...
    if (!tokenChanged) {
      ...
      const now = getDeimosCloudOAuthTokens()?.accessToken
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        const wsHeaders = {
          'User-Agent': getMCPUserAgent(),
          ...(serverRef.authToken && {
            'X-Deimos-Ide-Authorization': serverRef.authToken,
          }),
        }
        ...
        const { createChromeContext } = await import(
          '../../utils/deimosInChrome/mcpServer.js'
        )
        ...
        inProcessServer = createClaudeForChromeMcpServer(
          context,
        ) as InProcessMcpServer
        ...
        inProcessServer = (await createComputerUseMcpServerForCli()) as InProcessMcpServer
        const [clientTransport, serverTransport] = createLinkedTransportPair()
        await inProcessServer.connect(serverTransport)
        transport = clientTransport
      ...
      const client = new Client(
        {
          ...
          description: 'Deimos terminal coding agent (DXA)',
      ...
      return toolsToProcess
        .map((tool): Tool => {
          ...
          return {
            ...
            ...(isDeimosInChromeMCPServer(client.name) &&
              (client.config.type === 'stdio' || !client.config.type)
              ? deimosInChromeToolRendering().getDeimosInChromeMCPToolOverrides(
                tool.name,
              )
              : {}),
            ...(feature('CHICAGO_MCP') &&
