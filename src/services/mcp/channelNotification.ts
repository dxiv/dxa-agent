 ...
 * Requires dxa.dev/deimos OAuth auth — API key users are blocked until
 ...
 * dxa.dev/deimos auth; console orgs have no admin opt-in surface yet.
 *
...
export function gateChannelServer(
  ...
  if (!getDeimosCloudOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires dxa.dev/deimos authentication (run /login)',
    }
  }
