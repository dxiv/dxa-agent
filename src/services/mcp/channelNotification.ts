 ...
 * Requires github.com/dxiv/dxa-deimos OAuth auth — API key users are blocked until
 ...
 * github.com/dxiv/dxa-deimos auth; console orgs have no admin opt-in surface yet.
 *
...
export function gateChannelServer(
  ...
  if (!getDeimosCloudOAuthTokens()?.accessToken) {
    return {
      action: 'skip',
      kind: 'auth',
      reason: 'channels requires github.com/dxiv/dxa-deimos authentication (run /login)',
    }
  }
