...
/** Redacts userinfo, bearer material, and JWT-shaped tokens from connection errors (CLI + JSON). */
export function sanitizeMcpDoctorConnectionError(message: string): string {
  let s = message
  ...
  s = s.replace(
    /([?&])(api[_-]?key|token|password|secret|access_token|refresh_token)=([^&\s#]+)/gi,
    '$1$2=[redacted]',
  )
  return s
}

function mcpUiRemediationSuffix(configPath?: string): string {
  const fileHint = configPath ? ` Check or edit the MCP entry in ${configPath}.` : ''
  return ` Open /mcp to review servers, status, and authentication.${fileHint}`
}

function enrichValidationRemediation(
  suggestion: string | undefined,
  configPath?: string,
): string | undefined {
  const suffix = mcpUiRemediationSuffix(configPath)
  if (!suggestion?.trim()) {
    return suffix.trim()
  }
  const base = suggestion.trim()
  const punct = /[.!?…]$/.test(base) ? '' : '.'
  return `${base}${punct}${suffix}`
}
...
export function findingsFromValidationErrors(
  ...
  return validationErrors.map(error => {
    ...
    return {
      ...
      remediation: enrichValidationRemediation(error.suggestion, error.file),
...
function getConfigSignature(config: ScopedMcpServerConfig): string {
  switch (config.type) {
    ...
    default: {
      ...
      return `${config.scope}:${config.type ?? 'stdio'}:unknown`
    }
  }
}
...
function buildShadowingFindings(definitions: McpDoctorDefinition[]): McpDoctorFinding[] {
  ...
  const configPaths = [...new Set(userEditable.map(d => d.sourcePath).filter(Boolean))]
  const configList =
    configPaths.length > 0 ? ` Related config: ${configPaths.join('; ')}.` : ''
  return [
    {
      blocking: false,
      ...
      remediation: `Remove or rename one of the duplicate definitions so only one scope owns this server name.${mcpUiRemediationSuffix()}${configList}`,
    ...
    },
    {
      ...
      remediation: `Inspect the shadowed definitions and delete or rename the ones you no longer need.${mcpUiRemediationSuffix()}${configList}`,
      serverName: active.name,
      severity: 'warn',
    },
  ]
  ...
  for (const definition of definitions) {
    if (definition.pendingApproval) {
      findings.push({
        ...
        remediation: `Approve this server in the project MCP approval flow (from /mcp or the trust prompt) before expecting it to load.${mcpUiRemediationSuffix(definition.sourcePath)}`,
        scope: 'project',
    ...
    if (definition.disabled) {
      findings.push({
        ...
        remediation: `Re-enable the server in /mcp before expecting it at runtime.${mcpUiRemediationSuffix(definition.sourcePath)}`,
        serverName: definition.name,
        severity: 'warn',
        sourcePath: definition.sourcePath,
      })
...
async function getLiveCheck(
  ...
  // Never log `connection` here: it includes full server config (URLs, headers) that may contain secrets.
  const connection = await deps.connectToServer(name, activeConfig)
  const durationMs = Date.now() - startedAt

  let liveCheck!: McpDoctorLiveCheck
  switch (connection.type) {
    case 'connected':
      liveCheck = { attempted: true, result: 'connected', durationMs }
      break
    case 'needs-auth':
      liveCheck = { attempted: true, result: 'needs-auth', durationMs }
      break
    case 'disabled':
      liveCheck = { attempted: true, result: 'disabled', durationMs }
      break
    case 'pending':
      liveCheck = { attempted: true, result: 'pending', durationMs }
      break
    case 'failed':
      liveCheck = {
        attempted: true,
        ...
        durationMs,
        error:
          typeof connection.error === 'string' && connection.error.length > 0
            ? sanitizeMcpDoctorConnectionError(connection.error)
            : connection.error,
      }
      break
  }
    ...
    return [
      {
        ...
        remediation: `Complete sign-in for this server from /mcp (use the product OAuth or auth flow only; do not paste tokens or secrets into chat). Then run this check again.${mcpUiRemediationSuffix(activeDefinition?.sourcePath)}`,
        serverName: name,
        severity: 'warn',
    ...
    return [
      {
        ...
        remediation: commandNotFound
          ? `Verify the configured command exists on PATH, or set an absolute path to the executable.${mcpUiRemediationSuffix(activeDefinition?.sourcePath)}`
          : `Check transport settings (URL host/path and headers) and network reachability without sharing secrets; fix the underlying error, then retry.${mcpUiRemediationSuffix(activeDefinition?.sourcePath)}`,
        serverName: name,
        severity: 'error',
        sourcePath: activeDefinition?.sourcePath,
      },
...
async function buildServerReport(
  ...
  if (definitions.length === 0 && !shouldAddObservedDefinition) {
    findings.push({
      ...
      remediation: `Confirm the server name and config scope, or add the server from /mcp or your MCP config files, then retry.${mcpUiRemediationSuffix()}`,
      serverName: name,
      severity: 'error',
    })
  }
