...
import { getGlobalDeimosFile } from '../../utils/env.js'
...
export function describeMcpConfigFilePath(scope: ConfigScope): string {
  switch (scope) {
    case 'user':
      return getGlobalDeimosFile()
    case 'project':
      return join(getCwd(), '.mcp.json')
    case 'local':
      return `${getGlobalDeimosFile()} [project: ${getCwd()}]`
    case 'dynamic':
      return 'Dynamically configured'
    case 'enterprise':
      return getEnterpriseMcpFilePath()
    case 'deimos':
      return 'Deimos Cloud connectors'
    default:
      return scope
  }
}
...
export function getScopeLabel(scope: ConfigScope): string {
  switch (scope) {
    ...
    case 'deimos':
      return 'Deimos Cloud connector config'
    default:
      return scope
  }
}
...
export function getProjectMcpServerStatus(
  ...
  // In non-interactive mode (SDK, deimos -p, piped input), there's no way to
...
export function getMcpServerScopeFromToolName(
  ...
  // Fallback: github.com/dxiv/dxa-deimos servers have normalized names starting with "claude_ai_"
...
export function extractAgentMcpServers(
  ...
  for (const [name, { config, sourceAgents }] of serverMap) {
    ...
    // Skip unsupported transport types (sdk, deimos-proxy, sse-ide, ws-ide)
