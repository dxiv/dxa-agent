...
import { getDeimosCloudOAuthTokens } from 'src/utils/auth.js'
...
type DeimosCloudMcpServersResponse = {
  data: DeimosCloudMcpServer[]
  has_more: boolean
 ...
 */
export const fetchDeimosCloudMcpConfigsIfEligible = memoize(
  async (): Promise<Record<string, ScopedMcpServerConfig>> => {
    try {
      ...
      // Check for user:mcp_servers scope directly instead of isDeimosCloudSubscriber().
...
/**
 * Clears the memoized cache for fetchDeimosCloudMcpConfigsIfEligible.
 * Call this after login so the next fetch will use the new auth tokens.
 */
export function clearDeimosCloudMcpConfigsCache(): void {
  fetchDeimosCloudMcpConfigsIfEligible.cache.clear?.()
 ...
 */
export function markDeimosCloudMcpConnected(name: string): void {
  saveGlobalConfig(current => {
    const seen = current.deimosCloudMcpEverConnected ?? []
    if (seen.includes(name)) return current
    return { ...current, deimosCloudMcpEverConnected: [...seen, name] }
  })
}

export function hasDeimosCloudMcpEverConnected(name: string): boolean {
  return (getGlobalConfig().deimosCloudMcpEverConnected ?? []).includes(name)
}
