 ...
 *
 * Syncs user settings and memory files across Deimos environments.
 *
...
function isUsingOAuth(): boolean {
  ...
  return Boolean(
    tokens?.accessToken && tokens.scopes?.includes(DEIMOS_CLOUD_INFERENCE_SCOPE),
  )
}

function getSettingsSyncEndpoint(): string {
  return `${getOauthConfig().BASE_API_URL}/api/deimos/user_settings`
}
