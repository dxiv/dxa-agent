...
const CLI_SYSPROMPT_PREFIX_VALUES = [
  DEFAULT_PREFIX,
  AGENT_SDK_DEIMOS_PRESET_PREFIX,
  AGENT_SDK_PREFIX,
] as const
...
export function getCLISyspromptPrefix(options?: {
  ...
  if (options?.isNonInteractive) {
    if (options.hasAppendSystemPrompt) {
      return AGENT_SDK_DEIMOS_PRESET_PREFIX
    }
...
function isAttributionHeaderEnabled(): boolean {
  if (isEnvDefinedFalsy(process.env.DEIMOS_ATTRIBUTION_HEADER)) {
    return false
  }
 ...
 * server verifies this token to confirm the request came from a real Deimos
...
export function getAttributionHeader(fingerprint: string): string {
  ...
  const entrypoint = process.env.DEIMOS_ENTRYPOINT ?? 'unknown'
