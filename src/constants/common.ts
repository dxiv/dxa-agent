...
export function getLocalISODate(): string {
  ...
  if (process.env.DEIMOS_OVERRIDE_DATE) {
    return process.env.DEIMOS_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.DEIMOS_OVERRIDE_DATE
    ? new Date(process.env.DEIMOS_OVERRIDE_DATE)
    : new Date()
