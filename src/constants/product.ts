...
export function getDeimosCloudPortalBaseUrl(
  ...
  if (isRemoteSessionLocal(sessionId, ingressUrl)) {
    return DEIMOS_CLOUD_WEB_LOCAL_BASE_URL
  }
  if (isRemoteSessionStaging(sessionId, ingressUrl)) {
    return DEIMOS_CLOUD_WEB_STAGING_BASE_URL
  }
  return DEIMOS_CLOUD_WEB_BASE_URL
}
...
export function getRemoteSessionUrl(
  ...
  const baseUrl = getDeimosCloudPortalBaseUrl(compatId, ingressUrl)
