...
import { getExternalDeimosMdIncludes, getMemoryFiles, shouldShowDeimosMdExternalIncludesWarning } from './utils/deimosmd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], deimosInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    if (usesAnthropicSetup) {
      ...
      if (await shouldShowDeimosMdExternalIncludesWarning()) {
        const externalIncludes = getExternalDeimosMdIncludes(await getMemoryFiles(true));
        const {
          DeimosMdExternalIncludesDialog
        } = await import('./components/DeimosMdExternalIncludesDialog.js');
        await showSetupDialog(root, done => <DeimosMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
      }
    }
  }
  ...
  // processes but ignored by Deimos itself (see auth.ts).
  ...
  if (feature('KAIROS') || feature('KAIROS_CHANNELS')) {
    ...
    if (devChannels && devChannels.length > 0) {
      ...
      if (!isChannelsEnabled() || !getDeimosCloudOAuthTokens()?.accessToken) {
  ...
  // Show Chrome onboarding for first-time Deimos in Chrome users
  if (deimosInChrome && !getGlobalConfig().hasCompletedDeimosInChromeOnboarding) {
    const {
      DeimosInChromeOnboarding
    } = await import('./components/DeimosInChromeOnboarding.js');
    await showSetupDialog(root, done => <DeimosInChromeOnboarding onDone={done} />);
  }
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.DEIMOS_FRAME_TIMING_LOG;
  return {
