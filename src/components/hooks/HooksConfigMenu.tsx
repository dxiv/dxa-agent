 ...
 * or ask Deimos — the menu directs them there.
 *
...
export function HooksConfigMenu(t0) {
  ...
  const hooksByEventAndMatcher: Record<HookEvent, Record<string, IndividualHookConfig[]>> = t4;
  let t5;
  ...
  if (hooksDisabled_1) {
    ...
    if ($[53] !== disabledByPolicy) {
      t32 = !disabledByPolicy && <Text dimColor={true}>To re-enable hooks, remove "disableAllHooks" from settings.json or ask Deimos.</Text>;
      $[53] = disabledByPolicy;
      $[54] = t32;
    } else {
      t32 = $[54];
