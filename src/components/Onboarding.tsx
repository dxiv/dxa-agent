...
const CHEATSHEET_MD_ABSOLUTE = resolve(import.meta.dirname, '..', '..', 'docs', 'CHEATSHEET.md');
const CHEATSHEET_MD_FILE_URL = existsSync(CHEATSHEET_MD_ABSOLUTE)
  ? pathToFileURL(CHEATSHEET_MD_ABSOLUTE).href
  : null;
...
export function Onboarding({
  ...
  const securityStep = <Box flexDirection="column" gap={1} paddingLeft={1}>
      ...
      <Box flexDirection="column" width={70}>
        ...
        <OrderedList>
          <OrderedList.Item>
            <Text>Deimos can make mistakes</Text>
            <Text dimColor wrap="wrap">
              You should always review Deimos&apos;s responses, especially when
              <Newline />
              running code.
              <Newline />
            </Text>
          ...
          <OrderedList.Item>
            <Text>API keys stay local</Text>
            <Text dimColor wrap="wrap">
              Credentials are kept on this machine for normal use; avoid pasting
              secrets into prompts or tools you do not trust.
              <Newline />
            </Text>
          </OrderedList.Item>
          <OrderedList.Item>
            <Text>Review tool permissions</Text>
            <Text dimColor wrap="wrap">
              Check what Deimos may run before you approve. Use /permissions or
              Settings to tighten the permission mode when you want more manual
              control.
              <Newline />
            </Text>
          </OrderedList.Item>
      ...
      <Text dimColor wrap="wrap">
        More tips:{' '}
        {CHEATSHEET_MD_FILE_URL ? (
          <Link url={CHEATSHEET_MD_FILE_URL}>docs/CHEATSHEET.md</Link>
        ) : (
          <>see README</>
        )}
      </Text>
  ...
  if (shouldOfferTerminalSetup()) {
    steps.push({
      ...
      <Text bold>Use Deimos&apos;s terminal setup?</Text>
          <Box flexDirection="column" width={70} gap={1}>
            <Text>
              For the best experience, enable the recommended settings
              <Newline />
              for your terminal:{' '}
