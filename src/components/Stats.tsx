...
/** Default typings describe a single series; we plot multiple series (number[][]). */
const plotAsciiChart = asciichart as unknown as (
  series: number[][],
  options: {
    height?: number;
    colors?: string[];
...
import { aggregateDeimosCodeStatsForRange, type DeimosCodeStats, type DailyModelTokens, type StatsDateRange } from '../utils/stats.js';
...
/** Ant/external build flag — widened so TS accepts the comparison (see IS_ANT pattern). */
...
function createAllTimeStatsPromise(): Promise<StatsResult> {
  return aggregateDeimosCodeStatsForRange('all').then((data): StatsResult => {
...
export function Stats(t0) {
  ...
  if ($[1] === Symbol.for("react.memo_cache_sentinel")) {
    t2 = <Box marginTop={1}><Spinner /><Text> Loading your Deimos stats…</Text></Box>;
    $[1] = t2;
  } else {
    t2 = $[1];
  }
...
function StatsContent(t0) {
  ...
  const [statsCache, setStatsCache] =
    useState<Partial<Record<StatsDateRange, DeimosCodeStats>>>(t1);
  ...
  if ($[1] !== dateRange || $[2] !== statsCache) {
    t2 = () => {
      ...
      aggregateDeimosCodeStatsForRange(dateRange).then(data => {
        if (!cancelled) {
          setStatsCache(prev => ({
            ...prev,
            [dateRange]: data
  ...
  if (allTimeResult.type === "empty") {
    ...
    if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Box marginTop={1}><Text color="warning">No stats available yet. Start using Deimos!</Text></Box>;
      $[15] = t7;
    } else {
      t7 = $[15];
    }
...
function OverviewTab({
  ...
  stats: DeimosCodeStats;
  allTimeStats: DeimosCodeStats;
...
function generateFunFactoid(stats: DeimosCodeStats, totalTokens: number): string {
...
function generateTokenChart(dailyTokens: DailyModelTokens[], models: string[], terminalWidth: number): ChartOutput | null {
  ...
  const chart = plotAsciiChart(series, {
    height: 8,
...
async function handleScreenshot(stats: DeimosCodeStats, activeTab: 'Overview' | 'Models', setStatus: (status: string | null) => void): Promise<void> {
  setStatus('copying…');
...
function renderStatsToAnsi(stats: DeimosCodeStats, activeTab: 'Overview' | 'Models'): string {
  const lines: string[] = [];
...
function renderOverviewToAnsi(stats: DeimosCodeStats): string[] {
  const lines: string[] = [];
  const theme = getTheme(resolveThemeSetting(getGlobalConfig().theme));
...
function renderModelsToAnsi(stats: DeimosCodeStats): string[] {
  const lines: string[] = [];
  const modelEntries = Object.entries(stats.modelUsage).sort(([, a], [, b]) => b.inputTokens + b.outputTokens - (a.inputTokens + a.outputTokens));
  if (modelEntries.length === 0) {
