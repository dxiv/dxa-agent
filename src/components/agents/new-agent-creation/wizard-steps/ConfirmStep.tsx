...
export function ConfirmStep(t0) {
  ...
  const agent = wizardData.finalAgent as Omit<CustomAgentDefinition, 'location'>;
  let T0;
  let T1;
  let t10;
  let t11;
  ...
  if ($[4] !== agent || $[5] !== existingAgents || $[6] !== handleKeyDown || $[7] !== tools || $[8] !== wizardData.location) {
    ...
    if ($[39] !== agent.agentType || $[40] !== wizardData.location) {
      t25 = getNewRelativeAgentFilePath({
        source: wizardData.location as SettingSource | 'built-in',
    ...
    if ($[54] === Symbol.for("react.memo_cache_sentinel")) {
      t12 = <Box marginTop={1}><Text><Text bold={true}>Description</Text> (tells Deimos when to use this agent):</Text></Box>;
      $[54] = t12;
    } else {
      t12 = $[54];
    }
