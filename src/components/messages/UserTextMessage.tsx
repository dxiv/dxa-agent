...
export function UserTextMessage(t0) {
  ...
  if (feature("KAIROS_GITHUB_WEBHOOKS")) {
    if (param.text.startsWith("<github-webhook-activity>")) {
      ...
      const mod = t1 as Record<string, React.ComponentType<{
        addMargin: boolean;
        param: TextBlockParam;
      }>>;
      const UserGitHubWebhookMessage = mod.UserGitHubWebhookMessage;
      if (UserGitHubWebhookMessage == null) {
        return null;
      }
      let t2;
  ...
  if (feature("FORK_SUBAGENT")) {
    if (param.text.includes("<fork-boilerplate>")) {
      ...
      const UserForkBoilerplateMessage = mod.UserForkBoilerplateMessage;
      if (UserForkBoilerplateMessage == null) {
        return null;
      }
      let t2;
  ...
  if (feature("UDS_INBOX")) {
    if (param.text.includes("<cross-session-message")) {
      ...
      const UserCrossSessionMessage = mod.UserCrossSessionMessage;
      if (UserCrossSessionMessage == null) {
        return null;
      }
      let t2;
