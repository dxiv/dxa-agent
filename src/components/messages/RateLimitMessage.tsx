...
import { getRateLimitTier, getSubscriptionType, isDeimosCloudSubscriber } from 'src/utils/auth.js';
...
export function RateLimitMessage(t0) {
  ...
  if ($[2] === Symbol.for("react.memo_cache_sentinel")) {
    t3 = shouldProcessMockLimits() || isDeimosCloudSubscriber();
    $[2] = t3;
  } else {
    t3 = $[2];
  }
  ...
  const isCurrentlyRateLimited = deimosUsageLimits.status === "rejected" && deimosUsageLimits.resetsAt !== undefined && !deimosUsageLimits.isUsingOverage;
  ...
  bb0: {
    ...
    if ($[7] !== shouldAutoOpenRateLimitOptionsMenu) {
      t7 = getUpsellMessage({
        ...
        isTeamOrEnterprise,
        hasBillingAccess: hasDeimosCloudBillingAccess()
      });
