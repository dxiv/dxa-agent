...
/** Progress payloads read from `ProgressMessage.data` (typed loosely at the source). */
type ReplToolCallProgress = {
  type: 'repl_tool_call';
  phase: string;
  toolInput?: { command?: string; pattern?: string; file_path?: string };
  toolName?: string;
};
...
type CollapsedCommitRow = { kind: 'committed' | 'amended' | 'cherry-picked'; sha: string };
type CollapsedPushRow = { branch: string };
type CollapsedBranchRow = { action: 'merged' | 'rebased'; ref: string };
type CollapsedPrRow = {
...
export function CollapsedReadSearchContent({
  ...
  if (isActiveGroup) {
    for (const id_0 of toolUseIds) {
      ...
      const latestRaw = lookups.progressMessagesByToolUseID.get(id_0)?.at(-1)?.data;
      const latest = latestRaw as ReplToolCallProgress | undefined;
  ...
  if (verbose) {
    ...
    for (const msg of groupMessages as (NormalizedMessage | GroupedToolUseMessage)[]) {
  ...
  if (isFullscreenEnvEnabled() && isActiveGroup) {
    ...
    for (const id_1 of toolUseIds) {
      ...
      const dataRaw = lookups.progressMessagesByToolUseID.get(id_1)?.at(-1)?.data;
      const data = dataRaw as ShellProgressData | undefined;
  ...
  if (isFullscreenEnvEnabled() && message.commits?.length) {
    const commits = message.commits as CollapsedCommitRow[];
    const byKind = {
      committed: 'committed',
  ...
  if (isFullscreenEnvEnabled() && message.pushes?.length) {
    const pushes = message.pushes as CollapsedPushRow[];
    const branches = uniq(pushes.map(p => p.branch));
    pushPart('push', 'pushed to', <Text bold>{branches.join(', ')}</Text>);
  }
  if (isFullscreenEnvEnabled() && message.branches?.length) {
    const branchRows = message.branches as CollapsedBranchRow[];
    const byAction = {
      merged: 'merged',
  ...
  if (isFullscreenEnvEnabled() && message.prs?.length) {
    const prRows = message.prs as CollapsedPrRow[];
    const verbs = {
      created: 'created',
      edited: 'edited',
      merged: 'merged',
  ...
  return <Box flexDirection="column" marginTop={1} backgroundColor={bg}>
      <Box flexDirection="row">
        ...
        <Text dimColor={!isActiveGroup}>
          ...
          message,
          isActiveGroup: Boolean(isActiveGroup),
          hasPrecedingParts: Boolean(hasPrecedingNonMem || memParts.length > 0)
        }) : null}
