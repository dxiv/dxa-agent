...
import { getMemoryFiles, type MemoryFileInfo } from '../../utils/deimosmd.js';
import { DEIMOS_MEMORY_FILENAME } from '../../constants/deimosPaths.js';
...
export function MemoryFileSelector(t0: Props) {
  ...
  const userMemoryPath = join(getDeimosConfigHomeDir(), DEIMOS_MEMORY_FILENAME);
  const projectMemoryPath = join(getOriginalCwd(), DEIMOS_MEMORY_FILENAME);
  ...
  const memoryOptions: OptionWithDescription<string>[] = allMemoryFiles.map(file => {
  ...
  const folderOptions: OptionWithDescription<string>[] = [];
  ...
  if (isAutoMemoryEnabled()) {
    ...
    if (feature("TEAMMEM") && teamMemPaths && teamMemPaths.isTeamMemoryEnabled()) {
      let t2;
