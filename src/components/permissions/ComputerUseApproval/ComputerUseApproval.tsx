...
function ComputerUseTccPanel(t0) {
  ...
  if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = <Text dimColor={true}>Grant the missing permissions in System Settings, then select "Try again". macOS may require you to restart Deimos after granting Screen Recording.</Text>;
    $[15] = t7;
  } else {
    t7 = $[15];
  }
...
type SentinelCategoryKey = 'shell' | 'filesystem' | 'system_settings';
const SENTINEL_WARNING: Record<SentinelCategoryKey, string> = {
...
function ComputerUseAppListPanel(t0) {
  ...
  if ($[3] !== request.requestedFlags) {
    t3 = ALL_FLAG_KEYS.filter(k => (request.requestedFlags as Record<string, boolean>)[k]);
  ...
  if ($[12] !== checked || $[13] !== onDone || $[14] !== request.apps || $[15] !== requestedFlagKeys) {
    t10 = function respond(allow) {
      ...
      const flags = {
        ...(DEFAULT_GRANT_FLAGS as Record<string, boolean>),
        ...(Object.fromEntries(requestedFlagKeys.map(_temp3)) as Record<string, boolean>)
      };
      onDone({
        granted,
        denied,
  ...
  if ($[21] !== checked || $[22] !== request.apps) {
    ...
    if ($[24] !== checked) {
      t14 = a_3 => {
        ...
        const sentinel = getSentinelCategory(resolved.bundleId) as SentinelCategoryKey | null | undefined;
        const isChecked = checked.has(resolved.bundleId);
        const sentinelNote =
          sentinel != null && sentinel in SENTINEL_WARNING ? SENTINEL_WARNING[sentinel] : null;
        return <Box key={resolved.bundleId} flexDirection="column"><Text>{"  "}{isChecked ? figures.circleFilled : figures.circle}{" "}{resolved.displayName}</Text>{sentinelNote ? <Text bold={true}>{"    "}{figures.warning} {sentinelNote}</Text> : null}</Box>;
      };
      $[24] = checked;
      $[25] = t14;
    } else {
