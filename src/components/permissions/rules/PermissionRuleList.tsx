...
type RuleSelectOption = OptionWithDescription<string>;
...
function PermissionRulesTab(t0) {
  ...
  if ($[0] !== t0) {
    ...
    if ($[10] === Symbol.for("react.memo_cache_sentinel")) {
      t8 = {
        allow: "Deimos won't ask before using allowed tools.",
        ask: "Deimos will always ask for confirmation before using these tools.",
        deny: "Deimos will always reject requests to use denied tools."
      };
      $[10] = t8;
    } else {
      t8 = $[10];
...
export function PermissionRuleList(t0: Props) {
  ...
  const [validatedRule, setValidatedRule] = useState<ValidatedRuleDraft | null>(null);
  ...
  if ($[11] !== allowRulesByKey || $[12] !== askRulesByKey || $[13] !== denyRulesByKey) {
    t6 = (tab, t7) => {
      ...
      const options: RuleSelectOption[] = [];
      ...
      const sortedRuleKeys = Array.from(rulesByKey.keys() as Iterable<string>).sort((a, b) => {
  ...
  if ($[89] === Symbol.for("react.memo_cache_sentinel")) {
    t28 = <Text>Deimos can read files in the workspace, and make edits when auto-accept edits is on.</Text>;
    $[89] = t28;
  } else {
    t28 = $[89];
  }
