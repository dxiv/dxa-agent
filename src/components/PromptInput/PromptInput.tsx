...
function PromptInput({
  ...
  const hasBgTaskPill = useMemo(() => (Object.values(tasks) as TaskState[]).some(t => isBackgroundTask(t) && !(('external' as string) === 'ant' && isPanelAgentTask(t))), [tasks]);
  ...
  const cachedTeams: TeamSummary[] = useMemo(() => {
    ...
    const teammateCount = count(Object.values(teamContext.teammates) as { name: string }[], t => t.name !== 'team-lead');
    return [{
  ...
  useEffect(() => {
    if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
      addNotification({
        ...
        text: 'This prompt will launch an ultraplan session in Deimos on the web',
        priority: 'immediate',
        timeoutMs: 5000
      });
    } else {
  ...
  useEffect(() => {
    if (isUltrareviewEnabled() && ultrareviewTriggers.length) {
      addNotification({
        ...
        text: 'Run /ultrareview after Deimos finishes to review these changes in the cloud',
        priority: 'immediate',
        timeoutMs: 5000
      });
    }
