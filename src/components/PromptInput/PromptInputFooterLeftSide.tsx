...
function ModeIndicator({
  ...
  const hasTeams =
    ...
    count(
      Object.values(
        teamContext.teammates as Record<string, { name: string }>,
      ),
      tm => tm.name !== 'team-lead',
    ) > 0;
  if (mode === 'bash') {
  ...
  const hasInProcessTeammates = !showSpinnerTree && hasBackgroundTasks && Object.values(tasks).some((t: TaskState) => t.type === 'in_process_teammate');
  const hasTeammatePills = hasInProcessTeammates || !showSpinnerTree && isViewingTeammate;

  // In remote mode (`deimos assistant`, --teleport) the agent runs elsewhere;
  ...
  const hasAnyInProcessTeammates = Object.values(tasks).some((t: TaskState) => t.type === 'in_process_teammate' && t.status === 'running');
  const hasRunningAgentTasks = Object.values(tasks).some((t: TaskState) => t.type === 'local_agent' && t.status === 'running');
