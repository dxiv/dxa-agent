...
export function ContextVisualization(t0) {
  ...
  let t_transparency;
  if ($[96] !== data.contextTransparency) {
    const ct = data.contextTransparency;
    t_transparency = <Box flexDirection="column" flexShrink={0} gap={0}>
        <Text dimColor={true}>Transcript slice: {ct.messagesInModelSlice} messages · ~{formatTokens(ct.approximateMessageTokensRounded)} tokens (rough estimate)</Text>
        <Text dimColor={true}>Compact boundary in session: {ct.conversationHasCompactBoundary ? "yes" : "no"}</Text>
        <Text dimColor={true}>{ct.compactCommandHint}</Text>
      </Box>;
    $[96] = data.contextTransparency;
    $[97] = t_transparency;
  } else {
    t_transparency = $[97];
  }
  let t18;
  if ($[79] !== T1 || $[80] !== t15 || $[81] !== t17 || $[82] !== t6 || $[83] !== t7 || $[84] !== t8 || $[85] !== t9 || $[98] !== t_transparency) {
    t18 = <T1 flexDirection={t6} paddingLeft={t7}>{t8}{t_transparency}{t9}{t15}{t17}</T1>;
    $[79] = T1;
    $[80] = t15;
    $[81] = t17;
    $[82] = t6;
