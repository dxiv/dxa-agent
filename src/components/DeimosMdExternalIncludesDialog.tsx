...
import type { ExternalDeimosMdInclude } from '../utils/deimosmd.js';
...
type Props = {
  ...
  externalIncludes?: ExternalDeimosMdInclude[];
};
export function DeimosMdExternalIncludesDialog(t0: Props) {
  const $ = _c(18);
  const {
    onDone,
    isStandaloneDialog,
  ...
  if ($[8] === Symbol.for("react.memo_cache_sentinel")) {
    t8 = <Text dimColor={true}>Important: Only use Deimos with files you trust. Accessing untrusted files may pose security risks{" "}<Link url="https://github.com/dxiv/dxa-deimos/docs/en/security" />{" "}</Text>;
    $[8] = t8;
  } else {
    t8 = $[8];
  }
...
function _temp4(include: ExternalDeimosMdInclude, i: number) {
  return <Text key={i} dimColor={true}>{"  "}{include.path}</Text>;
}
function _temp3(current_0: any) {
  return {
    ...current_0,
    hasDeimosMdExternalIncludesApproved: true,
    hasDeimosMdExternalIncludesWarningShown: true
  };
}
