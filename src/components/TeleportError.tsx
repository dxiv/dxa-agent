...
import { checkIsGitClean, checkNeedsDeimosCloudLogin } from 'src/utils/background/remote/preconditions.js';
...
export function TeleportError(t0) {
  ...
  const handleLoginWithDeimosCloud = t6;
  let t7;
  if ($[9] === Symbol.for("react.memo_cache_sentinel")) {
    t7 = value => {
      if (value === "login") {
        handleLoginWithDeimosCloud();
      } else {
        onCancel();
      }
    };
    ...
    case "needsLogin":
      {
        if (isLoggingIn) {
          ...
          if ($[14] !== handleLoginComplete) {
            t9 = <ConsoleOAuthFlow onDone={handleLoginComplete} mode="login" forceLoginMethod="deimos" />;
        ...
        if ($[17] === Symbol.for("react.memo_cache_sentinel")) {
          t10 = <Dialog title="Log in to Deimos" onCancel={onCancel}>{t9}<Select options={[{
              label: "Login with Deimos account",
              value: "login"
            }, {
              label: "Exit",
              value: "exit"
...
export async function getTeleportErrors(): Promise<Set<TeleportLocalErrorType>> {
  ...
  const [needsLogin, isGitClean] = await Promise.all([checkNeedsDeimosCloudLogin(), checkIsGitClean()]);
  if (needsLogin) {
