...
export function SpinnerWithVerb(props: Props): React.ReactNode {
  ...
  useMemo(() => isEnvTruthy(process.env.DEIMOS_BRIEF), []) : false;
...
function SpinnerWithVerbInner({
  ...
  const effectiveTip = contextTipsActive ? undefined : showClearTip && !nextTask ? 'Use /clear to start fresh when switching topics and free up context' : showBtwTip && !nextTask ? "Use /btw to ask a quick side question without interrupting Deimos's current work" : spinnerTip;
