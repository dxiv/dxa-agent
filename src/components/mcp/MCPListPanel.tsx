...
export function MCPListPanel(t0) {
  ...
  const agentServers: AgentMcpServerInfo[] = t2;
  ...
  let items;
  if ($[9] !== agentServers || $[10] !== deimosCloudServers || $[11] !== dynamicServers || $[12] !== serversByScope) {
    items = [];
    ...
    for (const server_0 of deimosCloudServers) {
      items.push({
        type: "server",
        server: server_0
      });
...
function _temp2(s_0) {
  return s_0.client.config.type === "deimos-proxy";
}
function _temp(s) {
  return s.client.config.type !== "deimos-proxy";
}
