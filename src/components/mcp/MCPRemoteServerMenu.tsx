...
import type { DeimosCloudServerInfo, HTTPServerInfo, SSEServerInfo } from './types.js';
import { handleReconnectError, handleReconnectResult } from './utils/reconnectHelpers.js';
type Props = {
  server: SSEServerInfo | HTTPServerInfo | DeimosCloudServerInfo;
...
export function MCPRemoteServerMenu({
  ...
  const [isDeimosCloudAuthenticating, setIsDeimosCloudAuthenticating] = useState(false);
  const [deimosCloudAuthUrl, setDeimosCloudAuthUrl] = useState<string | null>(null);
  const [isDeimosCloudClearingAuth, setIsDeimosCloudClearingAuth] = useState(false);
  const [deimosCloudClearAuthUrl, setDeimosCloudClearAuthUrl] = useState<string | null>(null);
  const [deimosCloudClearAuthBrowserOpened, setDeimosCloudClearAuthBrowserOpened] = useState(false);
  ...
  const handleDeimosCloudAuthComplete = React.useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        ...
        onComplete?.('Authentication successful, but server still requires authentication. You may need to manually restart Deimos.');
      } else {
  ...
  const handleDeimosCloudClearAuthComplete = React.useCallback(async () => {
    ...
    setDeimosCloudClearAuthBrowserOpened(false);
  ...
  useKeybinding('confirm:no', () => {
    ...
    context: 'Confirmation',
    isActive: isDeimosCloudClearingAuth
  });

  // Return key handling for authentication flows and 'c' to copy URL
  useInput((input, key) => {
    if (key.return && isDeimosCloudAuthenticating) {
      void handleDeimosCloudAuthComplete();
    }
    if (key.return && isDeimosCloudClearingAuth) {
      if (deimosCloudClearAuthBrowserOpened) {
        void handleDeimosCloudClearAuthComplete();
      } else {
        // First Enter: open the browser
        const connectorsUrl = `${getOauthConfig().DEIMOS_CLOUD_WEB_ORIGIN}/settings/connectors`;
        setDeimosCloudClearAuthUrl(connectorsUrl);
        setDeimosCloudClearAuthBrowserOpened(true);
        void openBrowser(connectorsUrl);
      }
    }
    if (input === 'c' && !urlCopied) {
      const urlToCopy = authorizationUrl || deimosCloudAuthUrl || deimosCloudClearAuthUrl;
      if (urlToCopy) {
  ...
  const handleDeimosCloudAuth = React.useCallback(async () => {
    const deimosCloudPortalBaseUrl = getOauthConfig().DEIMOS_CLOUD_WEB_ORIGIN;
    ...
    if (orgUuid && server.config.type === 'deimos-proxy' && server.config.id) {
      ...
      authUrl = `${deimosCloudPortalBaseUrl}/settings/connectors`;
    }
  ...
  if (isDeimosCloudAuthenticating) {
    return <Box flexDirection="column" gap={1} padding={1}>
        <Text color="claude">Authenticating with {server.name}…</Text>
        <Box>
          <Spinner />
        ...
        {deimosCloudAuthUrl && <Box flexDirection="column">
            <Box>
              <Text dimColor>
                If your browser doesn&apos;t open automatically, copy this URL
                manually{' '}
  ...
  if (isDeimosCloudClearingAuth) {
    return <Box flexDirection="column" gap={1} padding={1}>
        <Text color="claude">Clear authentication for {server.name}</Text>
        {deimosCloudClearAuthBrowserOpened ? <>
            <Text>
              Find the MCP server in the browser and click
              &quot;Disconnect&quot;.
            </Text>
            {deimosCloudClearAuthUrl && <Box flexDirection="column">
                <Box>
                  <Text dimColor>
                ...
                <Link url={deimosCloudClearAuthUrl} />
              </Box>}
            <Box marginLeft={3} flexDirection="column">
              <Text color="permission">
            ...
            <Text>
              This will open github.com/dxiv/dxa-deimos in the browser. Find the MCP server in
