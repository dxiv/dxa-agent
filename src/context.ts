...
export const getUserContext = memoize(
  async (): Promise<{
    ...
    // DEIMOS_DISABLE_CLAUDE_MDS: hard off, always.
    // --bare: skip auto-discovery (cwd walk), BUT honor explicit --add-dir.
    // --bare means "skip what I didn't ask for", not "ignore what I asked for".
    const shouldDisableDeimosMd =
      isEnvTruthy(process.env.DEIMOS_DISABLE_CLAUDE_MDS) ||
      (isBareMode() && getAdditionalDirectoriesForDeimosMd().length === 0)
    // Await the async I/O (readFile/readdir directory walk) so the event
    // loop yields naturally at the first fs.readFile.
    const deimosMd = shouldDisableDeimosMd
      ? null
      : getDeimosMds(filterInjectedMemoryFiles(await getMemoryFiles()))
    // Cache for the auto-mode classifier (yoloClassifier.ts reads this
    // instead of importing deimosmd.ts directly, which would create a
    // cycle through permissions/filesystem → permissions → yoloClassifier).
    setCachedDeimosMdContent(deimosMd || null)

    logForDiagnosticsNoPII('info', 'user_context_completed', {
      duration_ms: Date.now() - startTime,
      deimosmd_length: deimosMd?.length ?? 0,
      deimosmd_disabled: Boolean(shouldDisableDeimosMd),
    })
