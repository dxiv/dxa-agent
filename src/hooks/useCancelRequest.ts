...
export function CancelRequestHandler(props: CancelRequestHandlerProps): null {
  ...
  const handleCancel = useCallback(() => {
    ...
    // This takes precedence over queue management so users can always interrupt Deimos
    ...
    // Priority 2: Pop queue when Deimos is idle (no running task to cancel)
  ...
  const killAllAgentsAndNotify = useCallback((): boolean => {
    const tasks = store.getState().tasks as { [id: string]: TaskState }
