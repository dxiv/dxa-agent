...
import { hasDeimosCloudMcpEverConnected } from '../../services/mcp/anthropicCloudMcp.js';
  ...
  if ($[0] !== addNotification || $[1] !== mcpClients) {
    t2 = () => {
      try {
        ...
        const failedDeimosCloudClients = mcpClients.filter(_temp2);
        const needsAuthLocalServers = mcpClients.filter(_temp3);
        const needsAuthDeimosCloudServers = mcpClients.filter(_temp4);
        if (failedLocalClients.length === 0 && failedDeimosCloudClients.length === 0 && needsAuthLocalServers.length === 0 && needsAuthDeimosCloudServers.length === 0) {
          return;
        }
