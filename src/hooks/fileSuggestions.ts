...
/**
 * Gets additional files from Deimos config directories
 */
async function getDeimosConfigFiles(cwd: string): Promise<string[]> {
  const markdownFileArrays = await Promise.all(
    DEIMOS_CONFIG_DIRECTORIES.map(subdir =>
