          ...
          case "ask":
            {
              ...
              if (feature("BASH_CLASSIFIER") && result.pendingClassifierCheck && tool.name === BASH_TOOL_NAME && !appState.toolPermissionContext.awaitAutomatedChecksBeforeDialog) {
                ...
                if (speculativePromise) {
                  type BashSpeculativeRaceOutcome =
                    | { type: 'result'; result: ClassifierResult }
                    | { type: 'timeout' };
                  const raceResult = await Promise.race<BashSpeculativeRaceOutcome>([
                    speculativePromise.then((r): BashSpeculativeRaceOutcome => ({
                      type: 'result',
                      result: r
                    })),
                    new Promise<BashSpeculativeRaceOutcome>(resolve => {
                      setTimeout(resolve, 2000, { type: 'timeout' });
                    })
                  ]);
                  if (ctx.resolveIfAborted(resolve)) {
                    return;
