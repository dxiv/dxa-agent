...
/**
 * Surfaces plugin-install prompts driven by `<deimos-hint />` tags.
 */
...
export function useDeimosHintRecommendation() {
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(
            `[useDeimosHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`,
          )
          markShownThisSession()
        }
  ...
  return t3 as UseDeimosHintRecommendationResult
}
