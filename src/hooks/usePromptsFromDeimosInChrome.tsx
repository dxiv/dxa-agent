...
import { DEIMOS_IN_CHROME_MCP_SERVER_NAME, isTrackedDeimosInChromeTabId } from '../utils/deimosInChrome/common.js';
...
/**
 * A hook that listens for prompt notifications from the Deimos for Chrome extension,
 * enqueues them as user prompts, and syncs permission mode changes to the extension.
 */
export function usePromptsFromDeimosInChrome(mcpClients, toolPermissionMode) {
  const $ = _c(6);
  useRef(undefined);
  let t0;
  if ($[0] !== mcpClients) {
...
function findChromeClient(clients: MCPServerConnection[]): ConnectedMCPServer | undefined {
  return clients.find((client): client is ConnectedMCPServer => client.type === 'connected' && client.name === DEIMOS_IN_CHROME_MCP_SERVER_NAME);
}
