...
/** Minimal SSH session handle for REPL props (full type lives in ssh layer when built out). */
type SSHSession = {
  createManager: (opts: unknown) => unknown;
  getStderrTail: () => string;
  proc: { exitCode: number | null; signalCode: number | null };
  proxy: { stop: () => void };
};
...
import { getMemoryFiles } from '../utils/deimosmd.js';
...
type UseFrustrationDetectionHook = (messages: import('../types/message.js').Message[], isLoading: boolean, hasActivePrompt: boolean, otherSurveysOpen: boolean) => {
  state: 'closed' | string;
  handleTranscriptSelect: () => void;
};
// `require` conditions must use `('external' as string) === 'ant'` (not only
// `IS_ANT_BUILD`) so Bun's bundler can statically drop ant-only branches and
// skip resolving modules absent from the open tree.
const useFrustrationDetection: UseFrustrationDetectionHook = ('external' as string) === 'ant' ? require('../components/FeedbackSurvey/useFrustrationDetection.js').useFrustrationDetection as UseFrustrationDetectionHook : (_messages, _isLoading, _hasActivePrompt, _otherSurveysOpen) => ({
  state: 'closed',
  handleTranscriptSelect: () => { }
});
// Ant-only org warning. Conditional require so the org UUID list is
// eliminated from external builds (one UUID is on excluded-strings).
const useAntOrgWarningNotification: () => void = ('external' as string) === 'ant' ? require('../hooks/notifs/useAntOrgWarningNotification.js').useAntOrgWarningNotification as () => void : () => { };
...
import { usePromptsFromDeimosInChrome } from 'src/hooks/usePromptsFromDeimosInChrome.js';
...
import { useDeimosHintRecommendation } from 'src/hooks/useDeimosHintRecommendation.js';
import { PluginHintMenu } from 'src/components/DeimosHint/PluginHintMenu.js';
...
const TungstenLiveMonitorComponent = TungstenLiveMonitor as unknown as React.ComponentType | null;
const WebBrowserPanel = WebBrowserPanelModule && typeof WebBrowserPanelModule === 'object' && 'WebBrowserPanel' in WebBrowserPanelModule ? (WebBrowserPanelModule as {
...
// repin to bottom. Josh Rosen's workflow: Deimos emits long output → scroll
...
/** Placeholder UI for ultraplan approval flow (full implementation is feature-gated). */
function UltraplanChoiceDialog(_props: {
  ...
  getAppState: () => import('../state/AppStateStore.js').AppState;
...
export function REPL({
  ...
  const titleDisabled = useMemo(() => isEnvTruthy(process.env.DEIMOS_DISABLE_TERMINAL_TITLE), []);
  const moreRightEnabled = useMemo(() => IS_ANT_BUILD && isEnvTruthy(process.env.CLAUDE_MORERIGHT), []);
  const disableVirtualScroll = useMemo(() => isEnvTruthy(process.env.DEIMOS_DISABLE_VIRTUAL_SCROLL), []);
  const disableMessageActions = feature('MESSAGE_ACTIONS') ?
    // biome-ignore lint/correctness/useHookAtTopLevel: feature() is a compile-time constant
    useMemo(() => isEnvTruthy(process.env.DEIMOS_DISABLE_MESSAGE_ACTIONS), []) : false;
  ...
  // from DEIMOS_NO_FLICKER=0 (which is process-lifetime) — this is
  ...
  usePromptsFromDeimosInChrome(isRemoteSession ? EMPTY_MCP_CLIENTS : mcpClients, toolPermissionContext.mode);
  ...
  // This enables "immediate" commands (like /btw) to persist while Deimos is processing.
  ...
  const terminalTitle = sessionTitle ?? agentTitle ?? haikuTitle ?? 'deimos';
  ...
  // Prevent macOS from sleeping while Deimos is working
  useEffect(() => {
  ...
  // This allows Deimos to edit files that were read in previous sessions
  ...
  function getFocusedInputDialog(): 'message-selector' | 'sandbox-permission' | 'tool-permission' | 'prompt' | 'worker-sandbox-permission' | 'elicitation' | 'cost' | 'idle-return' | 'init-onboarding' | 'ide-onboarding' | 'model-switch' | 'undercover-callout' | 'effort-callout' | 'remote-callout' | 'lsp-recommendation' | 'plugin-hint' | 'desktop-upsell' | 'ultraplan-choice' | 'ultraplan-launch' | undefined {
    ...
    if (IS_ANT_BUILD && allowDialogsWithAnimation && showModelSwitchCallout) return 'model-switch';

    // Undercover auto-enable explainer (internal-only, eliminated from external builds)
    if (IS_ANT_BUILD && allowDialogsWithAnimation && showUndercoverCallout) return 'undercover-callout';
  ...
  const getToolUseContext = useCallback((messages: MessageType[], newMessages: MessageType[], abortController: AbortController, mainLoopModel: string): ProcessUserInputContext => {
    ...
    return {
      ...
      setResponseLength,
      pushApiMetricsEntry: IS_ANT_BUILD ? (ttftMs: number) => {
        const now = Date.now();
  ...
  const handleBackgroundQuery = useCallback(() => {
    ...
    void (async () => {
      ...
      const uniqueNotifications = notificationMessages.filter(m => {
        ...
        return att.type === 'queued_command' && (typeof att.prompt !== 'string' || !existingPrompts.has(att.prompt));
      });
  ...
  const onQueryEvent = useCallback((event: Parameters<typeof handleMessageFromStream>[0]) => {
    handleMessageFromStream(event, newMessage => {
      if (isCompactBoundaryMessage(newMessage)) {
      ...
      } else if (newMessage.type === 'progress' && isEphemeralToolProgress((newMessage as ProgressMessage<{
        type: string;
      }>).data.type)) {
        ...
        setMessages(oldMessages => {
          ...
          const newProg = newMessage as ProgressMessage<{
            type: string;
          }>;
          if (last?.type === 'progress' && last.parentToolUseID === newProg.parentToolUseID && (last as ProgressMessage<{
            type: string;
          }>).data.type === newProg.data.type) {
  ...
  const onQueryImpl = useCallback(async (messagesIncludingNewMessages: MessageType[], newMessages: MessageType[], abortController: AbortController, shouldQuery: boolean, additionalAllowedTools: string[], mainLoopModelParam: string, effort?: EffortValue) => {
    ...
    // Mark onboarding as complete when any user message is sent to Deimos
    ...
    // title silently fell through to the "Deimos" default.
    ...
    if (IS_ANT_BUILD && apiMetricsRef.current.length > 0) {
      const entries = apiMetricsRef.current;
  ...
  const onQuery = useCallback(async (newMessages: MessageType[], abortController: AbortController, shouldQuery: boolean, additionalAllowedTools: string[], mainLoopModelParam: string, onBeforeQueryCallback?: (input: string, newMessages: MessageType[]) => Promise<boolean>, input?: string, effort?: EffortValue): Promise<void> => {
    ...
    try {
      ...
      if (queryGuard.end(thisGeneration)) {
        ...
        if (IS_ANT_BUILD && !abortController.signal.aborted) {
          setAppState(prev => {
  ...
  useEffect(() => {
    ...
    async function processInitialMessage(initialMsg: NonNullable<typeof pending>) {
      ...
      const shouldStorePlanForVerification = initialMsg.message.planContent && IS_ANT_BUILD && isEnvTruthy(undefined);
      setAppState(prev => {
  ...
  const onSubmit = useCallback(async (input: string, helpers: PromptInputHelpers, speculationAccept?: {
    ...
    // even while Deimos is processing. Commands opt-in via `immediate: true`.
    ...
    // controls treatment: "dialog" (blocking), "hint" (notification), "off".
    {
      ...
      const idleThresholdMin = Number(process.env.DEIMOS_IDLE_THRESHOLD_MINUTES ?? 75);
      const tokenThreshold = Number(process.env.DEIMOS_IDLE_TOKEN_THRESHOLD ?? 100_000);
  ...
  // Handlers for auto-run /issue or /good-deimos (defined after onSubmit)
  ...
  const handleSurveyRequestFeedback = useCallback(() => {
    const command = IS_ANT_BUILD ? '/issue' : '/feedback';
    onSubmit(command, {
      setCursorOffset: () => { },
      clearBuffer: () => { },
      resetHistory: () => { },
  ...
  // Show notification when Deimos is done responding and user is idle
  useEffect(() => {
    // Don't set up notification if Deimos is busy
    if (isLoading) return;
    ...
    const timer = setTimeout((lastQueryCompletionTime, isLoading, toolJSX, focusedInputDialogRef, terminal) => {
      ...
      if (lastUserInteraction > lastQueryCompletionTime) {
        // User has interacted since Deimos finished - they're not idle, don't notify
        return;
      }
      ...
      if (!isLoading && !toolJSX &&
        ...
        void sendNotification({
          message: 'Deimos is waiting for your input',
  ...
  useEffect(() => {
    ...
    const idleThresholdMs = Number(process.env.DEIMOS_IDLE_THRESHOLD_MINUTES ?? 75) * 60_000;
  ...
  useEffect(() => {
    const handleSuspend = () => {
      ...
      process.stdout.write(`\nDeimos has been suspended. Run \`fg\` to bring Deimos back.\nNote: ctrl + z now suspends Deimos, ctrl + _ undoes input.\n`);
    };
    const handleResume = () => {
  ...
  const stopHookSpinnerSuffix = useMemo(() => {
    ...
    const progressMsgs = messages.filter((m): m is ProgressMessage<HookProgress> => {
      ...
      const d = (m as ProgressMessage<HookProgress>).data;
      return d.type === 'hook_progress' && (d.hookEvent === 'Stop' || d.hookEvent === 'SubagentStop');
    });
    ...
    <MCPConnectionManager key={remountKey} dynamicMcpConfig={dynamicMcpConfig} isStrictMcpConfig={strictMcpConfig}>
      <FullscreenLayout scrollRef={scrollRef} overlay={toolPermissionOverlay} bottomFloat={isBuddyEnabled() && companionVisible && !companionNarrow ? <CompanionFloatingBubble /> : undefined} modal={centeredModal} modalScrollRef={modalScrollRef} dividerYRef={dividerYRef} hidePill={!!viewedAgentTask} hideSticky={!!viewedTeammateTask} newMessageCount={unseenDivider?.count ?? 0} onPillClick={() => {
        ...
        {/* Hide the processing placeholder while a modal is showing —
                  ...
                  the user sees their input echoed while Deimos processes. */}
        ...
        {IS_ANT_BUILD && TungstenLiveMonitorComponent ? <TungstenLiveMonitorComponent /> : null}
        {feature('WEB_BROWSER_TOOL') && WebBrowserPanel ? <WebBrowserPanel /> : null}
        <Box flexGrow={1} />
        ...
        <Box flexDirection="column" flexGrow={1}>
          ...
          {IS_ANT_BUILD && focusedInputDialog === 'model-switch' && AntModelSwitchCallout && <AntModelSwitchCallout onDone={(selection: string, modelAlias?: string) => {
          ...
          {IS_ANT_BUILD && focusedInputDialog === 'undercover-callout' && UndercoverAutoCallout && <UndercoverAutoCallout onDone={() => setShowUndercoverCallout(false)} />}
          ...
          {!toolJSX?.shouldHidePromptInput && !focusedInputDialog && !isExiting && !disabled && !cursor && !isShuttingDown() && <>
            ...
            {postCompactSurvey.state !== 'closed' ? <FeedbackSurvey state={postCompactSurvey.state} lastResponse={postCompactSurvey.lastResponse} handleSelect={postCompactSurvey.handleSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={handleSurveyRequestFeedback} /> : memorySurvey.state !== 'closed' ? <FeedbackSurvey state={memorySurvey.state} lastResponse={memorySurvey.lastResponse} handleSelect={memorySurvey.handleSelect} handleTranscriptSelect={memorySurvey.handleTranscriptSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={handleSurveyRequestFeedback} message="How well did Deimos use its memory? (optional)" /> : <FeedbackSurvey state={feedbackSurvey.state} lastResponse={feedbackSurvey.lastResponse} handleSelect={feedbackSurvey.handleSelect} handleTranscriptSelect={feedbackSurvey.handleTranscriptSelect} inputValue={inputValue} setInputValue={setInputValue} onRequestFeedback={didAutoRunIssueRef.current ? undefined : handleSurveyRequestFeedback} />}
            ...
            {IS_ANT_BUILD && skillImprovementSurvey.suggestion && <SkillImprovementSurvey isOpen={skillImprovementSurvey.isOpen} skillName={skillImprovementSurvey.suggestion.skillName} updates={skillImprovementSurvey.suggestion.updates} handleSelect={skillImprovementSurvey.handleSelect} inputValue={inputValue} setInputValue={setInputValue} />}
