...
import { getDeimosConfigHomeDir } from 'src/utils/envUtils.js';
...
export function Doctor(t0: Props) {
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        const userAgentsDir = join(getDeimosConfigHomeDir(), "agents");
        const projectAgentsDir = join(getOriginalCwd(), ".deimos", "agents");
        const {
          activeAgents,
          allAgents,
          failedFiles
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("Deimos diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
