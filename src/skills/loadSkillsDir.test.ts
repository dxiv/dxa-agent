...
function writeSkill(rootDir: string, skillPath: string): void {
  const skillDir = join(rootDir, '.deimos', 'skills', ...skillPath.split('/'))
...
test('loads flat and nested skills with colon namespaces', async () => {
  const configDir = mkdtempSync(join(tmpdir(), 'deimos-skills-'))
  const cwd = join(configDir, 'workspace')
  const originalConfigDir = process.env.DEIMOS_CONFIG_DIR
  ...
  try {
    ...
    process.env.DEIMOS_CONFIG_DIR = configDir
    clearSkillCaches()
    ...
    assert.equal(nestedSkill.skillRoot, join(configDir, '.deimos', 'skills', 'git', 'commit'))
    ...
    assert.equal(
      deepSkill.skillRoot,
      join(configDir, '.deimos', 'skills', 'frontend', 'react', 'form'),
    )
  } finally {
    if (originalConfigDir === undefined) {
      delete process.env.DEIMOS_CONFIG_DIR
    } else {
      process.env.DEIMOS_CONFIG_DIR = originalConfigDir
    }
    clearSkillCaches()
