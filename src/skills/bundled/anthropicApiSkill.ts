...
// anthropicApiSkillContent.js bundles ~250KB of .md strings. Lazy-load inside
// getPromptForCommand so they only enter memory when /anthropic-api is invoked.
type SkillContent = typeof import('./anthropicApiSkillContent.js')
...
→ Refer to \`{lang}/messages-api/README.md\`

**Chat UI or real-time response display:**
→ Refer to \`{lang}/messages-api/README.md\` + \`{lang}/messages-api/streaming.md\`

**Long-running conversations (may exceed context window):**
→ Refer to \`{lang}/messages-api/README.md\` — see Compaction section

**Prompt caching / optimize caching / "why is my cache hit rate low":**
→ Refer to \`shared/prompt-caching.md\` + \`{lang}/messages-api/README.md\` (Prompt Caching section)

**Function calling / tool use / agents:**
→ Refer to \`{lang}/messages-api/README.md\` + \`shared/tool-use-concepts.md\` + \`{lang}/messages-api/tool-use.md\`

**Batch processing (non-latency-sensitive):**
→ Refer to \`{lang}/messages-api/README.md\` + \`{lang}/messages-api/batches.md\`

**File uploads across multiple requests:**
→ Refer to \`{lang}/messages-api/README.md\` + \`{lang}/messages-api/files-api.md\`
...
export function registerAnthropicApiSkill(): void {
  registerBundledSkill({
    name: 'anthropic-api',
    description:
      'Build apps with the Anthropic Messages API or Anthropic SDK.\n' +
      'TRIGGER when: code imports `anthropic`/`@anthropic-ai/sdk`/`claude_agent_sdk`, or user asks to use the Anthropic API, Anthropic SDKs, or Agent SDK.\n' +
      'DO NOT TRIGGER when: code imports `openai`/other AI SDK, general programming, or ML/data-science tasks.',
    allowedTools: ['Read', 'Grep', 'Glob', 'WebFetch'],
    userInvocable: true,
    async getPromptForCommand(args) {
      const content = await import('./anthropicApiSkillContent.js')
