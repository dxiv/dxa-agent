...
\`\`\`
Plugin syntax: \`plugin-name@source\` where source is \`deimos-marketplace\`, \`claude-plugins-official\`, or \`builtin\`.
...
Hooks run commands at specific points in Deimos's lifecycle.
...
| Stop | - | Run when Deimos stops (including clear, resume, compact) |
...
Modify Deimos configuration by updating settings.json files.
...
User: "Format my code after Deimos writes it"
...
export function registerUpdateConfigSkill(): void {
  registerBundledSkill({
    name: 'update-config',
    description:
      'Use this skill to configure the Deimos harness via settings.json. Automated behaviors ("from now on when X", "each time X", "whenever X", "before/after X") require hooks configured in settings.json - the harness executes these, not Deimos, so memory/preferences cannot fulfill them. Also use for: permissions ("allow X", "add permission", "move permission to"), env vars ("set X=Y"), hook troubleshooting, or any changes to settings.json/settings.local.json files. Examples: "allow npm commands", "add bq permission to global settings", "move permission to user settings", "set DEBUG=true", "when claude stops show X". For simple settings like theme/model, use Config tool.',
    allowedTools: ['Read'],
    userInvocable: true,
