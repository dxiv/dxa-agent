...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.deimos', dir)
    case 'userSettings':
      return join(getDeimosConfigHomeDir(), dir)
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    const userSkillsDir = join(getDeimosConfigHomeDir(), 'skills')
    const managedSkillsDir = join(getManagedFilePath(), '.deimos', 'skills')
    ...
    const [
      ...
      isEnvTruthy(process.env.DEIMOS_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.deimos', 'skills')
