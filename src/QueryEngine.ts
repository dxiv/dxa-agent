...
import { accumulateUsage, updateUsage } from 'src/services/api/anthropicMessages.js'
...
export class QueryEngine {
  ...
  async *submitMessage(
    ...
    // a memory directory and needs Deimos to know how to use it (which
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.DEIMOS_EAGER_FLUSH) ||
          isEnvTruthy(process.env.DEIMOS_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via DEIMOS_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or DEIMOS_PLUGIN_SEED_DIR before this runs;
    ...
    if (!shouldQuery) {
      ...
      for (const msg of messagesFromUserInput) {
        ...
        if (msg.type === 'system' && msg.subtype === 'local_command') {
          const localCmd = msg as typeof msg & { content?: string }
          if (
            typeof localCmd.content === 'string' &&
            (localCmd.content.includes(`<${LOCAL_COMMAND_STDOUT_TAG}>`) ||
              localCmd.content.includes(`<${LOCAL_COMMAND_STDERR_TAG}>`))
          ) {
        ...
        if (msg.type === 'system' && msg.subtype === 'compact_boundary') {
          yield {
            ...
            session_id: getSessionId(),
            uuid: msg.uuid,
            compact_metadata: msg.compact_metadata as SDKCompactBoundaryMessage['compact_metadata'],
    ...
    for await (const message of query({
      ...
      if (
        ...
        if (
          ...
          const tailUuid =
            message.compact_metadata?.preserved_segment?.tail_uuid
          if (tailUuid) {
      ...
      switch (message.type) {
        ...
        case 'stream_event': {
          ...
          if (ev.type === 'message_start') {
            ...
            currentMessageUsage = updateUsage(
              currentMessageUsage,
              ev.message?.usage as Parameters<typeof updateUsage>[1],
            )
          }
          if (ev.type === 'message_delta') {
            currentMessageUsage = updateUsage(
              currentMessageUsage,
              ev.usage as Parameters<typeof updateUsage>[1],
            )
        ...
        case 'attachment': {
          const att = message.attachment as {
            type: string
            data?: unknown
            turnCount?: number
            maxTurns?: number
          ...
          if (att.type === 'structured_output') {
            structuredOutputFromTool = att.data
          }
          // Handle max turns reached signal from query.ts
          else if (att.type === 'max_turns_reached') {
            if (persistSession) {
              if (
                isEnvTruthy(process.env.DEIMOS_EAGER_FLUSH) ||
                isEnvTruthy(process.env.DEIMOS_IS_COWORK)
            ...
            yield {
              ...
              uuid: randomUUID(),
              errors: [
                `Reached maximum number of turns (${att.maxTurns})`,
              ],
            }
            return
          }
          // Yield queued_command attachments as SDK user message replays
          else if (replayUserMessages && att.type === 'queued_command') {
            yield {
              type: 'user',
              message: {
                role: 'user' as const,
              ...
              session_id: getSessionId(),
              parent_tool_use_id: null,
              uuid: att.source_uuid || message.uuid,
        ...
        case 'system': {
          ...
          if (
            ...
            yield {
              ...
              session_id: getSessionId(),
              uuid: message.uuid,
              compact_metadata: message.compact_metadata as SDKCompactBoundaryMessage['compact_metadata'],
            }
          }
          if (message.subtype === 'api_error') {
            const apiErr = message as typeof message & {
              retryAttempt?: number
              maxRetries?: number
              retryInMs?: number
              error?: { status?: number }
            ...
            yield {
              ...
              attempt: apiErr.retryAttempt ?? 0,
              max_retries: apiErr.maxRetries ?? 0,
              retry_delay_ms: apiErr.retryInMs ?? 0,
              error_status: apiErr.error?.status ?? null,
              error: categorizeRetryableAPIError(apiErr.error as APIError),
              session_id: getSessionId(),
              uuid: message.uuid,
            }
          }
...
/**
 * Sends a single prompt to the Deimos API and returns the response.
    ...
    ...(feature('HISTORY_SNIP')
      ? {
          snipReplay: (yielded: Message, store: Message[]) => {
            type SnipProjectionApi = {
              isSnipBoundaryMessage: (m: Message) => boolean
            }
            type SnipCompactApi = {
              snipCompactIfNeeded: (
                messages: Message[],
            ...
            if (
              !(snipProjection as SnipProjectionApi).isSnipBoundaryMessage(
                yielded,
              )
            )
              return undefined
            return (snipModule as unknown as SnipCompactApi).snipCompactIfNeeded(
              store,
              { force: true },
            )
          },
