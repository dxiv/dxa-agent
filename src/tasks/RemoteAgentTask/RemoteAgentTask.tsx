...
function sdkAssistantBlocks(
  msg: SDKMessage & { type: 'assistant' },
): readonly { type: string }[] {
  const content = (msg.message as { content?: unknown }).content;
  return Array.isArray(content) ? (content as readonly { type: string }[]) : [];
}
...
export function extractPlanFromLog(log: SDKMessage[]): string | null {
  ...
  for (let i = log.length - 1; i >= 0; i--) {
    ...
    const fullText = extractTextContent(sdkAssistantBlocks(msg), '\n');
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. Deimos never
...
function extractReviewFromLog(log: SDKMessage[]): string | null {
  ...
  const allText = log
    ...
    .map(msg => extractTextContent(sdkAssistantBlocks(msg), '\n'))
    .join('\n')
    .trim();
  return allText || null;
}
...
function extractTodoListFromLog(log: SDKMessage[]): TodoList {
  const todoListMessage = log.findLast((msg): msg is SDKAssistantMessage => {
    if (msg.type !== 'assistant') return false;
    return sdkAssistantBlocks(msg).some(
      b =>
        b.type === 'tool_use' &&
        'name' in b &&
        (b as ToolUseBlock).name === TodoWriteTool.name,
    );
  });
  if (!todoListMessage) {
    return [];
  }
  const input = sdkAssistantBlocks(todoListMessage).find(
    (block): block is ToolUseBlock =>
      block.type === 'tool_use' &&
      (block as ToolUseBlock).name === TodoWriteTool.name,
  )?.input;
  if (!input) {
    return [];
  }
...
function startRemoteSessionPolling(taskId: string, context: TaskContext): () => void {
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      // stableIdle is a prompt-mode completion signal (Deimos stops writing
