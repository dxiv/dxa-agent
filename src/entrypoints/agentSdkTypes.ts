/**
 * Main entrypoint for Deimos Agent SDK types.
 *
 ...
 *
 * If your SDK MCP calls will run longer than 60s, override DEIMOS_STREAM_CLOSE_TIMEOUT
 */
...
/**
 * A user message typed on github.com/dxiv/dxa-deimos, extracted from the bridge WS.
 * @internal
 */
...
/**
 * Hold a github.com/dxiv/dxa-deimos remote-control bridge connection from a daemon process.
 *
 ...
 * github.com/dxiv/dxa-deimos keeps the same session. Contrast with `query.enableRemoteControl`
 * which puts the WS in the CHILD process (dies with the agent).
 *
 * Pipe `query()` yields through `write()` + `sendResult()`. Read
 * `inboundPrompts()` (user typed on github.com/dxiv/dxa-deimos) into `query()`'s input
