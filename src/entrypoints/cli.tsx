import '../utils/deimosEnvAliases.js';
...
// Deimos: disable experimental API betas by default.
// Tool search (defer_loading), global cache scope, and context management
// require internal API support not available to external accounts → 500.
// Users can opt-in with DEIMOS_DISABLE_EXPERIMENTAL_BETAS=false.
// eslint-disable-next-line custom-rules/no-top-level-side-effects
process.env.DEIMOS_DISABLE_EXPERIMENTAL_BETAS ??= 'true'
...
if (process.env.DEIMOS_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.DEIMOS_ABLATION_BASELINE) {
  for (const k of ['DEIMOS_SIMPLE', 'DEIMOS_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'DEIMOS_DISABLE_AUTO_MEMORY', 'DEIMOS_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Deimos)`);
    return;
  }
  ...
  // Tab title must be set here — not in Commander `preAction` (runs only after
  // `main.js` is imported, so Windows Terminal could keep "claude" for seconds).
  const { startTerminalTitleTypewriter } = await import(
    '../utils/processTitleTypewriter.js'
  )
  startTerminalTitleTypewriter('deimos')
  ...
  if (process.argv[2] === '--claude-in-chrome-mcp') {
    ...
    const {
      runDeimosInChromeMcpServer
    } = await import('../utils/deimosInChrome/mcpServer.js');
    await runDeimosInChromeMcpServer();
    return;
  } else if (process.argv[2] === '--chrome-native-host') {
    profileCheckpoint('cli_chrome_native_host_path');
    const {
      runChromeNativeHost
    } = await import('../utils/deimosInChrome/chromeNativeHost.js');
  ...
  if (process.env.DEIMOS_DISABLE_EARLY_INPUT !== '1') {
    const {
