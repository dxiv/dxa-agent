 ...
 * return @path refs to prepend. Deimos's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches Deimos, just without @path.
 */
...
function uploadsDir(): string {
  return join(getDeimosConfigHomeDir(), 'uploads', getSessionId())
}
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // DEIMOS_CUSTOM_OAUTH_URL — keep it inside the try so a bad
