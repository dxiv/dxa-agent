 ...
 *
 * Used by both `deimos remote-control` (empty session so the user has somewhere to
...
export async function createBridgeSession({
  ...
  const { getDeimosCloudOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getDeimosCloudOAuthTokens()?.accessToken
  if (!accessToken) {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getDeimosCloudOAuthTokens()?.accessToken
  if (!accessToken) {
 ...
 * explicit client action. Both `deimos remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on github.com/dxiv/dxa-deimos.
 *
