...
export type EnvLessBridgeConfig = {
  ...
  min_version: string
  // When true, tell users their github.com/dxiv/dxa-deimos app may be too old to see v2
...
export async function checkEnvLessBridgeMinVersion(): Promise<string | null> {
  ...
  if (cfg.min_version && lt(MACRO.VERSION, cfg.min_version)) {
    return `Your version of Deimos (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${cfg.min_version} or higher is required. Run \`deimos /upgrade\` to update.`
  }
  return null
}

/**
 * Whether to nudge users toward upgrading their github.com/dxiv/dxa-deimos app when a
