...
A complete guide to running DXA Agent on Android using Termux + proot Ubuntu.
...
DXA Agent requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
You should see:
```
✓ Built dxa-agent v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to DXA Agent's large system prompt (~50K tokens):
